...
const envFile = mode === 'production' ? '.env.production' : (mode === 'canary' ? '.env.canary' : '.env.development');
...
const config: ForgeConfig = {
  ...
  hooks: {
    ...
    prePackage: async () => {
      ...
      if (isCI || isLinux || isDarwin) {
        console.log(`[prePackage] 检测到 ${process.platform} 环境，转换 workspace 依赖为实际目录 (Dereference)...`);
      ...
      const nativeModulesWithBin = ['sharp', 'better-sqlite3', 'electron-log'];
              ...
              for (const entry of entries) {
                ...
                try {
                  ...
                  if (stats.isSymbolicLink()) {
                    ...
                    if (typeof linkTarget === 'string' && /\.\.[/\\]\.\.[/\\]\.\./.test(linkTarget)) {
        ...
        for (const moduleName of nativeModulesWithBin) {
          ...
          if (fs.existsSync(binPath)) {
            try {
              ...
              logToFile(`[packageAfterPrune] Failed to remove .bin from ${moduleName}: ${e.message}`);
            }
          }
        }
        ...
        if (fs.existsSync(globalBinPath)) {
          try {
            ...
            if (hasExternalLinks) {
              ...
              logToFile(`[packageAfterPrune] Removed node_modules/.bin with external links: ${globalBinPath}`);
            }
          } catch (e) {
    ...
    postPackage: async (forgeConfig, packageResult) => {
      ...
      // 仅在目标平台为 macOS (darwin) 或 Linux 时执行权限修复
      const isDarwin = platform === 'darwin';
      const isLinux = platform === 'linux';
      
      if (isDarwin || isLinux) {
        console.log(`[postPackage] 正在处理目标平台 ${platform} 的二进制文件权限...`);
        
        for (const outputPath of outputPaths) {
          // macOS 的资源目录在 .app/Contents/Resources
          // Linux 的资源目录在 resources
          const appName = "yonuc-ai-folder";
          const resourcesDir = isDarwin 
            ? path.join(outputPath, `${appName}.app`, 'Contents', 'Resources')
          ...
          if (fs.existsSync(binDir)) {
            ...
            console.warn(`[postPackage] 未找到 bin 目录，跳过权限修复: ${binDir}`);
          }
        }
      }
    },
  ...
  packagerConfig: {
    ...
    // ASAR 打包配置
    asar: {
      unpackDir: "{node_modules/sharp,node_modules/@img,node_modules/better-sqlite3,node_modules/pdf-poppler}",
    },

    // extraResources 配置 - 包含llama-server二进制文件和配置文件
    extraResource: [
      "build/extraResources/bundles",
      ...
      "build/extraResources/bin",            // 包含 fastfetch 等二进制文件
      absAssetsDir,
    ],
    ignore: (file: string) => {
      ...
      if (relativePath === 'node_modules' || relativePath.startsWith('node_modules/')) {
        ...
        // 2. 检查是否在 EXTERNAL_DEPENDENCIES 白名单中
        const isExternal = EXTERNAL_DEPENDENCIES.some(dep => 
      ...
      const ignorePatterns = [
        ...
        (relativePath: string) => {
          if (relativePath.includes('node_modules/@img/sharp')) {
            ...
            // 如果路径包含其他平台或架构的标识，则忽略
            // 注意：这里需要精确匹配，避免误删
            if (relativePath.includes(platformPrefix)) {
              if (!relativePath.includes(`${platformPrefix}${targetSuffix}`)) {
                return true;
              }
            }
          }
        ...
        // 排除不属于当前平台和架构的额外二进制资源 (build/extraResources/bin 和 build/extraResources/bundles)
        ...[(relativePath: string) => {
          if (relativePath.startsWith('build/extraResources/bin/')) {
            const platformArch = `${process.platform}-${process.arch}`;
            // 确保保留 bin 根目录和当前平台的子目录
            if (relativePath === 'build/extraResources/bin') return false;
            if (relativePath.startsWith(`build/extraResources/bin/${platformArch}`)) return false;
            // 忽略其他平台的目录
            return true;
          }
          
          // 过滤 bundles 目录，根据 ai-engine 进一步筛选 (解决 Point 6, 7, 8)
          if (relativePath.startsWith('build/extraResources/bundles/')) {
            // 确保保留 bundles 根目录
            if (relativePath === 'build/extraResources/bundles') return false;
            
            const fileName = path.basename(relativePath).toLowerCase();
            const platform = process.platform;
            const arch = process.arch;
            const engine = packageJson['ai-engine'] || 'llama.cpp';

            // 1. 基础平台过滤
            let isPlatformMatch = false;
            if (platform === 'win32') {
              if (fileName.includes('-win-')) isPlatformMatch = true;
            } else if (platform === 'darwin') {
              if (fileName.includes('-macos-')) isPlatformMatch = true;
            } else if (platform === 'linux') {
              if (fileName.includes('-ubuntu-') || fileName.includes('-linux-')) isPlatformMatch = true;
            }
            if (!isPlatformMatch) return true;

            // 2. 架构过滤 (针对 Windows ARM64 vs X64)
            if (platform === 'win32') {
              if (arch === 'arm64') {
                // 如果是 arm64，排除所有包含 x64 或 x86 的包
                if (fileName.includes('x64') || fileName.includes('x86')) return true;
              } else if (arch === 'x64') {
                // 如果是 x64，排除所有包含 arm64 的包
                if (fileName.includes('arm64')) return true;
              }
            }
            ...
            if (engine === 'llama.cpp') {
              // 不需要 llamafile
              if (fileName.includes('llamafile')) return true;
              // 不需要 llama-completion (仅 llamafile 模式需要)
              if (fileName.includes('llama-completion')) return true;
              // 不需要 13.1 版本
              if (fileName.includes('13.1')) return true;
              // 只需要 12.4 版本 (针对 Windows CUDA)
              if (platform === 'win32' && fileName.includes('cuda') && !fileName.includes('12.4')) return true;
            } else if (engine === 'llamafile') {
              // 只需要 llamafile-*.zip 和补全依赖 (llama-completion)
              if (!fileName.includes('llamafile') && !fileName.includes('llama-completion')) return true;
            }
      ...
      const shouldIgnore = ignorePatterns.some(pattern => {
        if (typeof pattern === 'function') {
          return (pattern as any)(relativePath);
        }
