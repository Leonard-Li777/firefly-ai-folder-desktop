...
const config: ForgeConfig = {
    ...
    prePackage: async () => {
      ...
      if (aiEngine !== 'ollama') {
        ...
        const manifestPath = path.join(
          __dirname,
          `build/extraResources/configs/${manifestFileName}`
        )
        ...
        try {
          ...
          const presetBundlesDir = path.join(__dirname, 'build/presetResources', aiEngine)
          const errors: string[] = []
          ...
          const binDir = path.join(__dirname, 'build/extraResources/bin')
  ...
  packagerConfig: {
    ...
    asar: {
      unpackDir:
        '{node_modules/sharp,node_modules/@img,node_modules/better-sqlite3,node_modules/extract-file-icon,node_modules/@firecrawl,node_modules/exiftool-vendored,node_modules/exiftool-vendored.exe,node_modules/exiftool-vendored.pl}'
    },
