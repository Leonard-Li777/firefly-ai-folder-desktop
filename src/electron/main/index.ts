...
app.on('before-quit', async () => {
  ...
  // 优先清理底层的 ExifTool 及 Markitdown 活跃子进程，避免异步操作卡死导致孤儿进程残留
  try {
    const { MarkitdownProcessor } = await import('@firefly/core-engine')
    await MarkitdownProcessor.shutdown()

    const { markitdownServerManager } =
      await import('../runtime-services/system/markitdown-server-manager')
    markitdownServerManager.shutdown()
  } catch (e) {
    logger.error(LogCategory.MAIN, '清理 Markitdown 活跃进程失败:', e)
  }
...
app.on('will-quit', () => {
  try {
    const {
      markitdownServerManager
    } = require('../runtime-services/system/markitdown-server-manager')
    markitdownServerManager.shutdown()
  } catch {
    // ignore
  }
