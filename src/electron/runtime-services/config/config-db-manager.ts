...
export class ConfigDbManager {
  ...
  private getExtraResourcesDir(subdir: string): string {
    const basePath = app.isPackaged
      ...
      : (() => {
          // 开发/测试环境下 app.getAppPath() 可能指向 out_build/main 等构建产物目录，
          // 需要回退到源码目录下的 build/extraResources（与 config-adapter.ts 保持一致）
          let appPath = app.getAppPath()
          const standardPath = appPath.replace(/[\\/]/g, path.sep)
          const parts = standardPath.split(path.sep)
          if (parts.includes('out_build') || parts.includes('out')) {
            const outIndex = Math.max(parts.lastIndexOf('out_build'), parts.lastIndexOf('out'))
            if (outIndex > 0) {
              appPath = parts.slice(0, outIndex).join(path.sep)
            }
          }
          return path.join(appPath, 'build', 'extraResources')
        })()
    return path.join(basePath, subdir)
  }

  private getConfigFilePath(filename: string): string {
    return path.join(this.getExtraResourcesDir('configs'), filename)
  }
