...
import { LogCategory, logger, getMimeTypeByExtension } from '@yonuc/shared';
...
export class ModelCapabilityDetector extends EventEmitter implements IModelCapabilityDetector {
  ...
  private runtimeCapabilitiesTimestamp: number = 0;
  ...
  private statusCacheTimeout = 300000; // 5分钟 (原30秒)
   ...
   * 获取当前缓存的模型能力
   */
  public getCachedCapabilities(): AICapabilities | null {
   ...
   * 
   * @param modelId 模型 ID
   * @returns 模型能力信息或 null
   */
  public getModelCapabilityInfo(modelId: string): IModelCapabilityInfo | null {
    // 1. 尝试从缓存中获取
    if (this.capabilityCache.has(modelId)) {
      return this.capabilityCache.get(modelId)!;
    }

    // 2. 尝试从模型服务获取静态配置 (包含已转换的能力信息)
    const modelConfig = getLlamaModelConfig(modelId);
    if (modelConfig) {
      const capability: IModelCapabilityInfo = {
        modelId: modelConfig.id,
        supportedFileTypes: {}, // 这里简化处理，动态检测时会填充
        limitations: {
          maxContextLength: modelConfig.contextLength || 4096,
          maxFileSize: 100 // 默认值
        },
        performance: {
          avgProcessingTime: 0,
      ...
      // 填充支持的文件类型
      modelConfig.capabilities.forEach((cap: IModelCapability) => {
        capability.supportedFileTypes[cap.type] = cap.supportedFormats;
      });
      
      // 存入缓存供下次使用
      this.capabilityCache.set(modelId, capability);
      return capability;
    }
  ...
  private initializeFileTypeMappings(): void {
    const rawMappings = [
      // 文本文件
      { extension: 'txt', capabilityType: 'TEXT', isPrimary: true, complexity: 1 },
      { extension: 'md', capabilityType: 'TEXT', isPrimary: true, complexity: 2 },
      { extension: 'pdf', capabilityType: 'TEXT', isPrimary: true, complexity: 3 },
      { extension: 'doc', capabilityType: 'TEXT', isPrimary: true, complexity: 3 },
      { extension: 'docx', capabilityType: 'TEXT', isPrimary: true, complexity: 3 },
      { extension: 'rtf', capabilityType: 'TEXT', isPrimary: false, complexity: 2 },
      { extension: 'html', capabilityType: 'TEXT', isPrimary: false, complexity: 2 },
      { extension: 'xml', capabilityType: 'TEXT', isPrimary: false, complexity: 2 },
      { extension: 'json', capabilityType: 'TEXT', isPrimary: false, complexity: 2 },

      // 图像文件
      { extension: 'jpg', capabilityType: 'IMAGE', isPrimary: true, complexity: 2 },
      { extension: 'jpeg', capabilityType: 'IMAGE', isPrimary: true, complexity: 2 },
      { extension: 'jpe', capabilityType: 'IMAGE', isPrimary: true, complexity: 2 },
      { extension: 'png', capabilityType: 'IMAGE', isPrimary: true, complexity: 2 },
      { extension: 'bmp', capabilityType: 'IMAGE', isPrimary: true, complexity: 1 },
      { extension: 'webp', capabilityType: 'IMAGE', isPrimary: true, complexity: 2 },
      { extension: 'gif', capabilityType: 'IMAGE', isPrimary: true, complexity: 2 },
      { extension: 'svg', capabilityType: 'IMAGE', isPrimary: false, complexity: 3 },
      { extension: 'tiff', capabilityType: 'IMAGE', isPrimary: true, complexity: 2 },
      { extension: 'tif', capabilityType: 'IMAGE', isPrimary: true, complexity: 2 },
      { extension: 'heic', capabilityType: 'IMAGE', isPrimary: true, complexity: 2 },

      // 音频文件
      { extension: 'mp3', capabilityType: 'AUDIO', isPrimary: true, complexity: 3 },
      { extension: 'wav', capabilityType: 'AUDIO', isPrimary: true, complexity: 2 },
      { extension: 'flac', capabilityType: 'AUDIO', isPrimary: true, complexity: 3 },
      { extension: 'aac', capabilityType: 'AUDIO', isPrimary: true, complexity: 3 },
      { extension: 'ogg', capabilityType: 'AUDIO', isPrimary: false, complexity: 3 },
      { extension: 'm4a', capabilityType: 'AUDIO', isPrimary: false, complexity: 3 },

      // 视频文件
      { extension: 'mp4', capabilityType: 'VIDEO', isPrimary: true, complexity: 4 },
      { extension: 'avi', capabilityType: 'VIDEO', isPrimary: true, complexity: 4 },
      { extension: 'mov', capabilityType: 'VIDEO', isPrimary: true, complexity: 4 },
      { extension: 'mkv', capabilityType: 'VIDEO', isPrimary: true, complexity: 4 },
      { extension: 'flv', capabilityType: 'VIDEO', isPrimary: true, complexity: 4 },
      { extension: 'webm', capabilityType: 'VIDEO', isPrimary: false, complexity: 4 },
      { extension: 'wmv', capabilityType: 'VIDEO', isPrimary: true, complexity: 4 }
    ] as const;

    this.fileTypeMappings = rawMappings.map(mapping => ({
      ...mapping,
      mimeType: getMimeTypeByExtension(mapping.extension),
      capabilityType: mapping.capabilityType as TModelCapabilityType
    }));
  }
  ...
  private async getModelConfig(modelId: string): Promise<ILlamaModelConfig | null> {
    return getLlamaModelConfig(modelId);
  }
   ...
   * 检测本地服务能力
   */
  public async detectLocalCapabilities(serviceConfig: ExtendedAIServiceConfig): Promise<AICapabilities> {
    const modelIdentifier = serviceConfig.local.modelId || serviceConfig.local.modelPath || 'unknown';

    const capabilities: AICapabilities = {
      supportsText: true,
      supportsImage: !!serviceConfig.local.mmprojPath,
      supportsAudio: false,
      supportsVideo: false,
      maxContextSize: 'contextSize' in serviceConfig.local ? serviceConfig.local.contextSize :
        'contextLength' in serviceConfig.local ? (serviceConfig.local as any).contextLength : 4096,
      modelName: modelIdentifier,
      provider: 'local'
    };

    // 根据模型配置补全能力（支持 IMAGE, AUDIO, VIDEO 等标记）
    const configCapTypes = serviceConfig.local.capabilities || [];
    if (configCapTypes.length > 0) {
      const caps = configCapTypes.map((c: any) => typeof c === 'string' ? c.toUpperCase() : c.type?.toUpperCase() || '');
      if (caps.includes('IMAGE')) capabilities.supportsImage = true;
      if (caps.includes('AUDIO')) capabilities.supportsAudio = true;
      if (caps.includes('VIDEO')) capabilities.supportsVideo = true;
    }

    // 根据模型名称额外判断（保留原有逻辑作为回退）
    if (modelIdentifier.toLowerCase().includes('omni')) {
  ...
  async checkRuntimeCapabilities(): Promise<IRuntimeModalityCapabilities> {
    const now = Date.now();
    const cacheAge = now - this.runtimeCapabilitiesTimestamp;

    // 检查缓存且未过期 (5分钟)
    if (this.runtimeCapabilitiesCache && cacheAge < 300000) {
      logger.debug(LogCategory.MODEL_CAPABILITY_DETECTOR, `[模型能力检测器] 使用缓存的运行时能力 (缓存年龄: ${Math.round(cacheAge / 1000)}秒)`);
    ...
    try {
      ...
      this.runtimeCapabilitiesTimestamp = Date.now();
  ...
  async checkFileTypeSupport(modelId: string, fileExtension: string): Promise<ICapabilityMatchResult> {
    ...
    if (!this.currentServiceConfig || cachedModelId !== modelId) {
      try {
        const freshConfig = await ConfigOrchestrator.getInstance().getAIConfig() as ExtendedAIServiceConfig;
    ...
    const capability = modelConfig.capabilities.find((cap: IModelCapability) => cap.type === fileMapping.capabilityType);
  ...
  async getCapabilityLimitations(modelId: string, capabilityType: TModelCapabilityType): Promise<string[]> {
    ...
    const capability = modelConfig.capabilities.find((cap: IModelCapability) => cap.type === capabilityType);
   ...
   * 分析模型能力
   */
  private async analyzeModelCapabilities(modelConfig: any): Promise<IModelCapabilityInfo> {
    const supportedFileTypes: { [key in TModelCapabilityType]?: string[] } = {};

    // 获取能力列表并统一为字符串数组
    const caps = modelConfig.capabilities || modelConfig.supportedFormats || [];
    const types = Array.isArray(caps) 
      ? caps.map((c: any) => typeof c === 'string' ? c.toUpperCase() : c.type?.toUpperCase() || '')
      : [];

    // 根据能力类型填充支持的文件类型（使用默认格式，不再依赖模型特定配置）
    types.forEach(type => {
      const normalizedType = type as TModelCapabilityType;
      // 动态导入或硬编码映射以获取默认格式
      supportedFileTypes[normalizedType] = this.getDefaultFormatsForType(normalizedType);
    });

    // 计算性能指标
    const performance = {
      avgProcessingTime: this.estimateAvgProcessingTime(modelConfig),
      memoryUsage: (modelConfig.vramRequiredGB || 0) * 1024,
  ...
  private async buildModelStatus(modelConfig: ILlamaModelConfig): Promise<IModelCapabilityStatus> {
    return {
      ...
      status: 'idle',
      availableCapabilities: modelConfig.capabilities.map((cap: IModelCapability) => cap.type),
      currentLimitations: {
        maxFileSize: this.calculateMaxFileSize(modelConfig),
        maxContextLength: modelConfig.contextLength || 32768,
