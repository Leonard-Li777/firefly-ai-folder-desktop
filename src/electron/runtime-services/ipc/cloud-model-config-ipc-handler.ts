...
export function registerCloudModelConfigIPCHandlers(): void {
  ...
  // 更新配置
  ipcMain.handle(
    'cloud-model-config:update-config',
    async (_, index: number, config: CloudModelConfig) => {
      try {
        logger.info(
          LogCategory.IPC,
          `处理IPC请求: 更新云端配置 index=${index} provider=${config.provider}`
        )
  ...
  ipcMain.handle(
    ...
    async (_, provider: string, apiKey: string, baseUrl?: string, throwOnError = false) => {
      try {
        logger.info(LogCategory.IPC, `处理IPC请求: 获取${provider}的模型列表`)
        const models = await cloudModelConfigService.getProviderModels(
          provider,
          apiKey,
          baseUrl,
          throwOnError
  ...
  ipcMain.handle('cloud-model-config:get-cloud-providers-config', async (_, language: string) => {
    try {
      logger.debug(LogCategory.IPC, `处理IPC请求: 获取云端提供商配置列表 language=${language}`)
      unifiedModelManager.ensureLoaded()
      const configs = ConfigOrchestrator.getInstance().getValue<any[]>('CLOUD_MODEL_CONFIGS') || []
      return { success: true, data: configs }
    } catch (error) {
      const message = error instanceof Error ? error.message : t('未知错误')
      logger.error(LogCategory.IPC, `获取云端提供商配置列表失败 language=${language}:`, error)
      return { success: false, error: message }
    }
  })
