...
export function registerOllamaIPCHandlers() {
  ...
  ipcMain.handle('ollama:get-download-mirror', async () => {
    const mirror =
      ConfigOrchestrator.getInstance().getValue<'cn' | 'global'>('DOWNLOAD_MIRROR') || 'cn'
  ...
  ipcMain.handle('ollama:needs-setup', async () => {
    try {
      ...
      return { needsSetup: false, error: t('未知错误') }
    }
  })
  ...
  ipcMain.handle('ollama:check-model', async (_, modelId: string) => {
    try {
      ...
      return { installed: false, error: t('未知错误') }
    }
  })
  ...
  ipcMain.handle('ollama:open-website', async () => {
    try {
      await shell.openExternal('https://ollama.com/')
      return { success: true }
    } catch (error) {
      logger.error(LogCategory.IPC, '无法打开 Ollama 官网:', error)
      return { success: false, error: t('未知错误') }
    }
  })
