...
import * as shared from '@firefly/shared'
import { ModelDownloadManager, DownloadStatus } from '../ai/model-download-manager'
...
export class ModelDownloadManagerIPCHandler {
  ...
  private async registerHandlers(): Promise<void> {
    ...
    try {
      // ModelDownloadManager 已在顶部静态导入

      // 检查模型下载状态
      ipcMain.handle(
        'model-download-manager:check-status',
        async (_event, modelId: string, source?: string) => {
          try {
            safeLogger.debug(
              category,
              '[ModelDownloadManagerIPC] 检查模型下载状态:',
              modelId,
              source
            )
            const status = await ModelDownloadManager.getInstance().checkModelDownloadStatus(
              modelId,
              source
            )
      ...
      ipcMain.handle(
        ...
        async (event, modelId: string, options?: any) => {
          try {
            safeLogger.info(category, '[ModelDownloadManagerIPC] 开始下载模型:', modelId)

            const webContentsId = event?.sender?.id
            const task = await ModelDownloadManager.getInstance().startDownload(
              modelId,
              webContentsId,
              options
            )
      ...
      ipcMain.handle('model-download-manager:get-task-status', async (_event, taskId: string) => {
        try {
          ...
          if (!task) {
            return { success: false, error: t('任务不存在') }
          }
      ...
      ipcMain.handle(
        ...
        async (_event, modelId: string, source?: string) => {
          try {
            const task = await ModelDownloadManager.getInstance().getModelTask(modelId, source)
            if (!task) {
      ...
      ipcMain.handle(
        ...
        async (_event, modelId: string, source?: string) => {
          try {
            const isDownloading = ModelDownloadManager.getInstance().isModelDownloading(
              modelId,
              source
            )
