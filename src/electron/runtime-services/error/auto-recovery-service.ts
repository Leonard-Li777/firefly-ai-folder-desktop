...
import { AppError, ErrorType, RecoveryStrategy, SystemHealthStatus } from '@firefly/types'
...
export class AutoRecoveryService extends EventEmitter {
  ...
  private isRecoveryEnabled = true
  ...
  private maxRecoveryHistory = 1000
  ...
  private updateRecoveryStats(success: boolean, duration: number): void {
    ...
    // 更新平均恢复时间
    const totalTime =
      this.recoveryStats.averageRecoveryTime * (this.recoveryStats.totalAttempts - 1) + duration
