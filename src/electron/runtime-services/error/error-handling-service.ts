...
import { AppError, ErrorType, RecoveryStrategy, LogLevel } from '@firefly/types'
...
export class ErrorHandlingService extends EventEmitter {
   ...
   * 检查是否有重复错误
   */
  public hasDuplicateError(error: AppError, timeWindow = 60000): boolean {
    const now = Date.now()
