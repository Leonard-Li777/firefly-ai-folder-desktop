...
class FileWatcherService {
  ...
  async syncDirectory(dirPath: string, triggerAnalysis: boolean = false): Promise<void> {
    ...
    // 防止频繁重复同步：同一目录在 10 秒内不重复同步（从 2 秒增加到 10 秒）
    ...
    this.lastSyncTime.set(normalizedPath, now);

    this.syncingPaths.add(normalizedPath);

    try {
      logger.debug(LogCategory.FILE_WATCHER, `开始异步同步目录: ${normalizedPath}`);

      // 1. 获取工作空间信息
      const workspace = await databaseService.findRootWorkspaceDirectory(normalizedPath);
      ...
      const diskEntries = await fs.promises.readdir(normalizedPath, { withFileTypes: true });
      ...
      for (let i = 0; i < diskFiles.length; i += BATCH_SIZE) {
        ...
        await Promise.all(batch.map(async (file) => {
          ...
          if (!shouldIgnoreFile(standardPath, file.name, this.ignoreRules)) {
            try {
              const stats = await fs.promises.stat(fullPath);
              diskFileMap.set(standardPath, stats);
            } catch (e) {
              // 文件可能已消失
            }
          }
      ...
      // 4. 读取数据库记录
      const dbFiles = await databaseService.getFilesByParentPath(normalizedPath, workspace.id);
      const dbFileMap = new Map<string, any>();
      for (const file of dbFiles) {
        // 直接使用数据库路径作为键进行比对，V2.2 确保数据库存储的是标准路径
        dbFileMap.set(path.resolve(file.path), file);
      }
      ...
      for (const [filePath, stats] of diskFileMap.entries()) {
        ...
        if (!dbFile) {
          ...
          await this.handleFileAdded(workspace.id, normalizedPath, filePath, false);
          changed = true;
        } else {
          // 检查文件是否真的发生了变化
          // 1. 检查大小
          ...
          if (sizeChanged || timeChanged) {
            logger.debug(LogCategory.FILE_WATCHER, `文件发生变更 (大小:${sizeChanged}, 时间:${timeChanged}): ${filePath}`);
            // 传递工作目录的自动监听状态，决定是否重新分析
            await this.handleFileChanged(workspace.id, normalizedPath, filePath, !!workspace.autoWatch);
            changed = true;
          }
        }
  ...
  private async handleFileAdded(workspaceId: number, directoryPath: string, filePath: string, autoWatchEnabled?: boolean): Promise<void> {
    ...
    // 【防护1】检查是否正在处理此文件
    if (this.processingFiles.has(standardPath)) {
    ...
    try {
      ...
      // 【防护3】标记为正在处理
      this.processingFiles.add(standardPath);
      ...
      const fileId = await databaseService.addFileFromPath(standardPath, directoryPath, workspaceId, true);
      ...
      if (isAutoWatchActive) {
        ...
        const fileExt = path.extname(standardPath).toLowerCase();
      ...
      this.processingFiles.delete(standardPath);
    }
  }
  ...
  private async handleFileChanged(workspaceId: number, directoryPath: string, filePath: string, autoWatchEnabled?: boolean): Promise<void> {
    ...
    try {
      ...
      await databaseService.updateFileModifiedTime(standardPath, stats.mtime);
      logger.debug(LogCategory.FILE_WATCHER, `文件修改时间已更新: ${filePath}`);

      const file = await databaseService.getFileByPath(standardPath);
      if (!file) {
      ...
      if (isAutoWatchActive) {
        const contentHash = await databaseService.calculateFileFingerprint(standardPath)
