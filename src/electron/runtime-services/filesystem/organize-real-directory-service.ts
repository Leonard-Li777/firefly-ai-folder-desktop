...
export class OrganizeRealDirectoryService {
  ...
  private async organizeFileWithHardlinks(operation: FileOperation): Promise<FileOrganizeResult> {
    try {
      ...
      // 1. 鲁棒性检查：直接检查原始路径是否存在
      if (!fs.existsSync(actualOldPath)) {
        // 2. 如果文件不存在，可能是时序问题（文件已被当前整理流程中的其他虚拟目录移动）
        //    检查文件是否已经被移动到目标路径
    ...
    try {
      ...
      if (files.length > 0) {
        ...
        const pathDepths = files.map(f => {
          const relativePath = f.path.replace(workspaceDirectoryPath, '').replace(prefix, '')
        ...
        `
          ...
          .all(`${prefix}%`, `${prefix.replace(/\\/g, '/')}%`) as any[]
  ...
  async generateOrganizePlan(
    ...
    try {
      ...
      if (options?.filePaths && options.filePaths.length > 0) {
        // 遵循禁止归一化原则，直接使用原始路径匹配
        const selectedPaths = new Set(options.filePaths)
        analyzedFiles = analyzedFiles.filter(f => selectedPaths.has(f.path))
...
`
      ...
      const aiServiceMode = ConfigOrchestrator.getInstance().getValue<string>('AI_SERVICE_MODE') || 'local'
      ...
      const temperature = ConfigOrchestrator.getInstance().getValue<number>('MODEL_TEMPERATURE') || 0.3
      ...
      if (error.message === 'CancellationRequested') {
        logger.info(LogCategory.FILE_ORGANIZATION, '用户取消了生成一键整理方案')
      } else {
  ...
  async quickOrganize(
    ...
    try {
      ...
      const buildDirectoryPaths = (directories: DirectoryNode[]): Map<string, string> => {
        const pathMap = new Map<string, string>() // nodeIdentifier -> relativePath
        ...
        while (maxIterations > 0 && pathMap.size < directories.length) {
          for (const dir of directories) {
            ...
            if (dir.parent && pathMap.has(dir.parent)) {
              ...
              pathMap.set(nodeIdentifier, path.join(parentPath, dir.name))
            }
          }
      ...
      const fileOperations: { fileId: number, fileName: string, sourcePath: string, targetPath: string, smartName: string }[] = []

      // 遍历所有目录，创建目录并记录文件目标路径
      logger.info(LogCategory.FILE_ORGANIZATION, '[一键整理] 开始解析方案结构', { 
        workspaceDirectoryPath,
        directoryCount: aiGeneratedStructure.directories.length 
      })
      ...
      const workspace = this.db.prepare('SELECT workspace_id, path FROM workspaces WHERE path = ? OR path = ?').get(normalizedPath, normalizedPath + path.sep) as { workspace_id: number, path: string } | undefined
      ...
      for (const dir of aiGeneratedStructure.directories) {
        ...
        if (dir.files) {
          for (const fileItem of dir.files) {
            ...
            logger.info(LogCategory.FILE_ORGANIZATION, '[一键整理] 正在处理文件项', { fileItem })
            
            // 跳过重复文件
            let existingFile: { id: number, name: string, sourcePath: string, smartName: string } | undefined

            const fileId = typeof fileItem === 'object' ? fileItem.id : undefined
            if (fileId) {
              existingFile = this.db.prepare(
              ...
              ).get(Number(fileId)) as { id: number, name: string, sourcePath: string, smartName: string } | undefined
            }
            ...
            if (!existingFile) {
              ...
              ).get(fileName, fileName, `${prefix}%`, workspaceDirectoryPath) as { id: number, name: string, sourcePath: string, smartName: string } | undefined
            }

            if (!existingFile) {
              logger.warn(LogCategory.FILE_ORGANIZATION, '[一键整理] 最终找不到文件信息', { fileName })
              continue
            }

            if (processedFileIds.has(existingFile.id)) {
              logger.warn(LogCategory.FILE_ORGANIZATION, '[一键整理] 文件重复，跳过', {
            ...
            const targetFileName = existingFile.smartName || existingFile.name
            ...
            fileOperations.push({
              ...
              targetPath,
              smartName: existingFile.smartName || existingFile.name
            })
          }
        }
      }
      ...
      for (const op of fileOperations) {
          ...
          try {
            ...
            logger.info(LogCategory.FILE_ORGANIZATION, `[一键整理] 执行移动: ${operation.oldPath} -> ${operation.newPath}`)
