...
import { logger, LogCategory, FileCategory, isCategory } from '@yonuc/shared'
...
export class ThumbnailService {
  ...
  private async generateThumbnailNative(
    ...
    try {
      ...
      const thumbnail = await (nativeImage as any).createThumbnailFromPath(normalizedPath, { width: size, height: size })
  ...
  private async generateThumbnailWithSharp(
    ...
    try {
      ...
      logger.info(LogCategory.FILE_PROCESSOR, `[缩略图服务] Sharp方法成功生成缩略图: ${outputPath}`)
      return true
    } catch (error) {
  ...
  private async convertOfficeToPDF(officePath: string, outputDir: string, fileId: string): Promise<string | null> {
    try {
      ...
      let libreOfficePath = ConfigOrchestrator.getInstance().getValue<string>('LIBREOFFICE_PATH');
      ...
      // 使用独立的临时用户目录，避免多实例冲突
      const userProfileDir = path.join(os.tmpdir(), `lo_user_${fileId}_${Date.now()}`);
      await fs.mkdir(userProfileDir, { recursive: true });
      const userProfileArg = `-env:UserInstallation=${pathToFileURL(userProfileDir).href}`;

      // 使用LibreOffice命令行转换临时文件
      // --headless: 不显示GUI
      // --convert-to pdf: 转换为PDF格式
      // --outdir: 输出目录
      const command = `"${libreOfficePath}" ${userProfileArg} --headless --convert-to pdf --outdir "${outputDir}" "${tempOfficePath}"`
      ...
      try {
        ...
        logger.warn(LogCategory.FILE_PROCESSOR, `[缩略图服务] 删除临时用户目录失败:`, e)
      }
  ...
  private getPopplerPath(): string | undefined {
    // 1. 生产环境：从 app.asar.unpacked 中获取
    if (app.isPackaged) {
      ...
      const exeName = process.platform === 'win32' ? 'pdftocairo.exe' : 'pdftocairo'
      if (fsSync.existsSync(path.join(unpackedPath, exeName))) {
        logger.info(LogCategory.FILE_PROCESSOR, `[缩略图服务] 生产环境找到Poppler路径: ${unpackedPath}`)
        return unpackedPath
      }
    }

    // 2. 开发环境：尝试寻找 node_modules/pdf-poppler
    // 尝试几个可能的路径，注意 Windows 需要包含 bin 目录
    const binDir = process.platform === 'win32' ? 'bin' : ''
    const possiblePaths = [
      // Monorepo root (relative to apps/desktop)
      path.resolve(process.cwd(), '../../node_modules/pdf-poppler/lib/win/poppler-0.51', binDir),
      // Current directory node_modules (fallback)
      path.resolve(process.cwd(), 'node_modules/pdf-poppler/lib/win/poppler-0.51', binDir),
      // Deep search
      path.resolve(__dirname, '../../../../../../node_modules/pdf-poppler/lib/win/poppler-0.51', binDir),
      path.resolve(__dirname, '../../../../../../../node_modules/pdf-poppler/lib/win/poppler-0.51', binDir)
    ]

    for (const p of possiblePaths) {
      const exeName = process.platform === 'win32' ? 'pdftocairo.exe' : 'pdftocairo'
      if (fsSync.existsSync(path.join(p, exeName))) {
        logger.info(LogCategory.FILE_PROCESSOR, `[缩略图服务] 开发环境找到Poppler路径: ${p}`)
        return p
      }
    }

    logger.warn(LogCategory.FILE_PROCESSOR, `[缩略图服务] 未找到Poppler路径，将使用库默认配置`)
    return undefined
  }
