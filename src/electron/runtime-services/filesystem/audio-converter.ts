...
export class AudioConverter {
  ...
  private constructor() {
    this.tempDir = path.join(app.getPath('temp'), 'yonuc-ai-audio-cache')
  ...
  async convertToStandard(inputPath: string): Promise<string> {
    ...
    if (!ffmpegPath) {
      throw new Error('FFmpeg 未就绪，无法转换音频')
    }

    // 从配置中获取截取时长，默认为 30 秒
    const DURATION_LIMIT = ConfigOrchestrator.getInstance().getValue<number>('AUDIO_ANALYSIS_DURATION') || 30
    ...
    return new Promise((resolve, reject) => {
      const command = `"${ffmpegPath}" -i "${inputPath}" -t ${DURATION_LIMIT} -ar 16000 -ac 1 -c:a pcm_s16le -y "${outputPath}"`
