...
const V2_2_SCHEMA = `
  ...
  CREATE TABLE IF NOT EXISTS files (
    ...
    created_at DATETIME NOT NULL,                -- 文件在文件系统中的创建时间
    modified_at DATETIME NOT NULL,               -- 文件在文件系统中的修改时间
    accessed_at DATETIME NOT NULL                -- 文件在文件系统中的最后访问时间
  );
  ...
  CREATE TABLE IF NOT EXISTS analysis_queue (
    ...
    created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, -- 任务创建时间
    updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP  -- 任务状态更新时间
  );
  ...
  -- 15. 虚拟目录表（基于标签/属性筛选的动态视图）
  CREATE TABLE IF NOT EXISTS virtual_directories (
    id TEXT PRIMARY KEY,                         -- 虚拟目录 ID
    name TEXT NOT NULL,                          -- 虚拟目录显示名
    description TEXT,                            -- 视图规则描述
    filters TEXT NOT NULL,                       -- 筛选器规则集 (JSON)
    parent_id TEXT,                              -- 嵌套父虚拟目录 ID
    workspace_id INTEGER NOT NULL,               -- 关联的基础工作区
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, -- 创建时间
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, -- 更新时间
