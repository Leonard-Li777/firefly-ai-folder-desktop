  ...
  describe('三表架构字段路由', () => {
    ...
    it('应该正确路由 workspace_files 表字段', () => {
      ...
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `).run(fingerprint, workspaceDir.id, workspaceDir.id, filePath, 'test-file.txt', 1)
  ...
  describe('文件操作时序', () => {
    it('应该在文件移动后正确更新路径', () => {
      ...
      db.prepare(`
        INSERT OR IGNORE INTO workspace_directories (path, name, type)
        VALUES (?, ?, 'SPEEDY')
      `).run(oldDir, 'Old Location')

      db.prepare(`
        INSERT OR IGNORE INTO workspace_directories (path, name, type)
        VALUES (?, ?, 'SPEEDY')
      `).run(newDir, 'New Location')
      ...
      db.prepare(`
        INSERT INTO workspace_files (file_fingerprint, workspace_id, directory_id, path, name, is_analyzed)
        VALUES (?, ?, ?, ?, ?, ?)
      `).run(fingerprint, oldDirRecord.id, oldDirRecord.id, oldPath, 'file.txt', 1)
