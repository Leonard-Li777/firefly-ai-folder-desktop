...
export class FileDao {
  ...
  async updateFilePath(oldPath: string, newPath: string): Promise<void> {
    ...
    // 查找原记录
    const wf = this.db.prepare('SELECT * FROM workspace_files WHERE path = ?').get(normalizedOldPath) as any;
    if (!wf) {
    ...
    this.db.transaction(() => {
      ...
      this.db.prepare('DELETE FROM workspace_files WHERE path = ?').run(normalizedOldPath);
      ...
      `).run(
        wf.file_fingerprint, wf.workspace_id, newDirId, normalizedNewPath, path.basename(normalizedNewPath),
        wf.is_analyzed, wf.analysis_error, wf.last_analyzed_at, wf.thumbnail_path, wf.created_at
      );

      // 注意：file_unit_relations 关联的是 workspace_files.id
      // 由于旧记录已删除、新记录有新 ID，需要更新关联
      // 但由于这是路径变更而非内容变更，Unit 关联应该保留
      // 实际应用中需要在调用此方法后由 Service 层处理 Unit 关联迁移
    })();
  }
  ...
  async getAllFiles(): Promise<any[]> {
    const rows = this.db.prepare(`
      SELECT f.*, wf.path, wf.name, wf.modified_at as wf_mod, wf.id
      FROM workspace_files wf
  ...
  async searchFilesFTS(query: string, workspaceId?: number): Promise<any[]> {
    ...
    if (trimmedQuery.length < 3) {
      let sql = `
        SELECT f.*, wf.id, wf.path, wf.name, wf.is_analyzed, wf.modified_at as wf_mod
        FROM workspace_files wf
        LEFT JOIN files f ON wf.file_fingerprint = f.file_fingerprint
        LEFT JOIN file_contents fc ON f.file_fingerprint = fc.file_fingerprint
        WHERE (wf.name LIKE ? OR f.smart_name LIKE ? OR fc.description LIKE ? OR fc.content LIKE ? OR fc.multimodal_content LIKE ? OR fc.lrc LIKE ?)
      `;
  ...
  async resetFileAnalysis(filePath: string): Promise<void> {
    try {
      // 增加路径格式化和日志，排查 Windows 路径大小写或格式问题
      ...
      const fileRecord = this.db.prepare(`
        ...
        FROM workspace_files wf
        WHERE wf.path = ?
      `).get(normalizedPath) as { id: number, file_fingerprint: string, path: string, is_analyzed: number } | undefined;
  ...
  async getFilesByParentPath(dirPath: string, workspaceId: number): Promise<any[]> {
    ...
    const rows = this.db.prepare(`
      ...
      WHERE wf.workspace_id = ?
        AND wf.path LIKE ? || '${sep}%'
        AND INSTR(SUBSTR(wf.path, LENGTH(?) + 2), '${sep}') = 0
      ORDER BY wf.name ASC
    `).all(workspaceId, dirPath, dirPath) as any[];
    ...
    return rows.map(row => ({
      ...
      size: row.size,
      type: row.type,
      extension: row.type, // 【关键修复】添加 extension 字段，用于缩略图显示
      mimeType: row.mime_type,
      isAnalyzed: row.is_analyzed === 1,
