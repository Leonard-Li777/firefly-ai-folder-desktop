...
export class FileAnalysisIntegrationService extends EventEmitter {
  ...
  private aiAdapter: LlamaIndexAIAdapter

  constructor() {
    super()
    this.aiAdapter = new LlamaIndexAIAdapter()
  }
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      logger.info(LogCategory.AI_SERVICE, '文件分析集成服务示例初始化完成')
    } catch (error) {
      logger.error(LogCategory.AI_SERVICE, '文件分析集成服务示例初始化失败:', error)
      throw error
    }
  }
  ...
  private async classifyAnalyzedFile(result: FileAnalysisResult): Promise<void> {
    try {
      ...
      // 构建提示词
      const prompt = `请分析以下文件内容并给出简短分类：\n文件名：${result.filename}\n内容：${contentPreview}`
      
      // 调用AI适配器进行推理
      const inferenceResult = await this.aiAdapter.inference({
        prompt
      })
      ...
      if (inferenceResult.success) {
        ...
        const enhancedResult = {
          ...result,
          classification: {
             summary: inferenceResult.response,
             category: 'auto'
          }
        }
        ...
        logger.info(LogCategory.AI_SERVICE, `文件分析推理完成: ${result.filename}`)
      }
    } catch (error) {
      logger.error(LogCategory.AI_SERVICE, `文件分类推理失败: ${result.filename}`, error)
    }
  }
