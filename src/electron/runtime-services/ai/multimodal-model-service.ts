...
export class MultiModalModelService extends EventEmitter {
   ...
   * 构建多模态模型信息
   */
  private async buildMultiModalInfo(modelConfig: any): Promise<IMultiModalModelInfo> {
    const fileGroups: IMultiModalFileGroup = {
      mainModel: modelConfig.files.find((f: any) => f.type === 'model')!,
      mmproj: modelConfig.files.find((f: any) => f.type === 'mmproj'),
      tokenizer: modelConfig.files.find((f: any) => f.type === 'tokenizer'),
      config: modelConfig.files.find((f: any) => f.type === 'config'),
      auxiliary: modelConfig.files.filter((f: any) =>
        f.type !== 'model' && f.type !== 'mmproj' && f.type !== 'tokenizer' && f.type !== 'config'
      )
    };

    const caps = modelConfig.capabilities || [];
    const supportedModalities = caps.map((cap: any) => typeof cap === 'string' ? cap : cap.type);
    ...
    // 构建依赖关系
    const dependencies = {
      required: modelConfig.files.filter((f: any) => f.required).map((f: any) => f.name),
      optional: modelConfig.files.filter((f: any) => !f.required).map((f: any) => f.name)
    };
