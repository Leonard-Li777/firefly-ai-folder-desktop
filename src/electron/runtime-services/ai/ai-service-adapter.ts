...
import { AIErrorType } from '@yonuc/types'
...
export class AIServiceAdapter implements IAIServiceAdapter {
  ...
  getAIService(): ILlamaIndexAIService {
    if (!this.aiService) {
      ...
      // 注册错误监听器，用于捕获超时等异常并通知用户
      if (this.aiService && (this.aiService as any).on) {
        (this.aiService as any).on('error', (error: any) => {
          const errorMsg = error instanceof Error ? error.message : String(error)
          
          // 判断是否为超时错误
          if (errorMsg.includes('timed out') || errorMsg.includes('timeout')) {
            logger.warn(LogCategory.AI_SERVICE, '[AIServiceAdapter] 捕获到 AI 服务超时错误，触发错误处理程序')
            aiErrorHandler.handleError(
              aiErrorHandler.createAIError(
                AIErrorType.REQUEST_TIMEOUT,
                errorMsg,
                'LlamaIndexAIService',
                { originalError: error }
              )
...
class UnifiedAIServiceManagerImpl implements IUnifiedAIServiceManager {
  ...
  async getCapabilities(): Promise<AICapabilities> {
    try {
      ...
      const { createModelCapabilityAdapter } = require('../../adapters/model-capability-adapter');
      const capabilities = await createModelCapabilityAdapter().getCapabilities()
