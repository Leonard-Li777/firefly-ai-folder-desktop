import { LogCategory, logger, getIsDebugMode } from '@yonuc/shared';
  ...
  private async performSync(): Promise<boolean> {
    ...
    try {
      ...
      if (pendingDimExp.length > 0) {
        ...
        await cloudAnalysisService.batchSync({ dimension_expansions: payload }, language);
      ...
      if (pendingTagExp.length > 0) {
        ...
        if (toSync.length > 0) {
          ...
          await cloudAnalysisService.batchSync({ tag_expansions: payload }, language);
        }
      ...
      if (relatedTags.length > 0) {
        // 仅推送非泛维度的标签定义
        const tagsToPush = relatedTags.filter(t => !panSet.has(Number(t.dimension_id)));
      ...
      const relationsPayload = fileTagLinks.map(link => {
        // 过滤泛维度关系
        if (panSet.has(Number(link.dimension_id))) return null;
      ...
      logger.info(LogCategory.SUPABASE, `CloudSyncWorker: 已同步 ${pendingFiles.length} 个文件及 ${relationsPayload.length} 个有效关联 (已过滤泛维度)`);
