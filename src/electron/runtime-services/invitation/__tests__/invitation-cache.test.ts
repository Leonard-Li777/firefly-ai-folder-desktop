...
describe('InvitationService Cache Logic', () => {
  ...
  it('当 Supabase 请求成功时，应该更新本地缓存', async () => {
    ...
    vi.mocked(configService.getSupabaseClient).mockReturnValue(mockSupabase as any)
    ...
    expect(configService.updateValue).toHaveBeenCalledWith('INVITATION_CACHE_DATA', expect.objectContaining({
      invitationCount: 5,
      isInvited: true
    }))
  })
  ...
  it('当 Supabase 请求失败时，应该回滚使用本地缓存', async () => {
    // 1. 模拟 Supabase 报错
    vi.mocked(configService.getSupabaseClient).mockReturnValue({
      from: () => ({
    ...
    vi.mocked(configService.getValue).mockReturnValue(mockCache)
