...
import { LicenseService, LicenseStatus } from '../system/license-service'
...
class InvitationService {
  ...
  async getInvitationQuota() {
    ...
    try {
      ...
      if (licenseResult.status === LicenseStatus.AUTHORIZED && licenseResult.type === 'ENTERPRISE_OFFLINE') {
  ...
  async checkQuotaLimit(): Promise<void> {
    // 1. 检查是否已解锁无限额度
    const isUnlocked = ConfigOrchestrator.getInstance().getValue<boolean>('IS_PRIVATE_DIRECTORY_UNLOCKED')
