...
export class ResourceManager extends EventEmitter {
    ...
    this.cleanupStrategies = [
      {
        ...
        trigger: resource =>
          resource.usage > this.monitoringConfig.thresholds[ResourceType.CONNECTION].warning,
      ...
      },
      {
        ...
        trigger: resource =>
          resource.usage > this.monitoringConfig.thresholds[ResourceType.MEMORY].warning,
      ...
      },
      {
        ...
        trigger: resource =>
          resource.usage > this.monitoringConfig.thresholds[resource.type].critical,
      ...
      },
      {
        ...
        trigger: resource =>
          resource.usage > this.monitoringConfig.thresholds[resource.type].exhausted,
  ...
  private applyConfigFromSettings(): void {
    const interval = ConfigOrchestrator.getInstance().getValue<number>('HARDWARE_CHECK_INTERVAL')
    if (typeof interval === 'number' && interval > 0) {
      this.monitoringConfig.monitoringInterval = interval
    }

    const enableMonitoring = ConfigOrchestrator.getInstance().getValue<boolean>(
  ...
  private registerConfigWatchers(): void {
    ...
    ConfigOrchestrator.getInstance().onValueChange('HARDWARE_CHECK_INTERVAL', value => {
      const interval =
        typeof value === 'number' && value > 0 ? value : this.monitoringConfig.monitoringInterval
      this.monitoringConfig.monitoringInterval = interval
  ...
  private updateThresholdsFromSettings(): void {
    this.setThresholdPercent(
      ResourceType.CPU,
      'warning',
      ConfigOrchestrator.getInstance().getValue<number>('CPU_WARNING_THRESHOLD')
    )
    this.setThresholdPercent(
      ResourceType.CPU,
      'critical',
      ConfigOrchestrator.getInstance().getValue<number>('CPU_CRITICAL_THRESHOLD')
    )
    this.setThresholdPercent(
      ResourceType.MEMORY,
      'warning',
      ConfigOrchestrator.getInstance().getValue<number>('MEMORY_WARNING_THRESHOLD')
    )
    this.setThresholdPercent(
      ResourceType.MEMORY,
      'critical',
      ConfigOrchestrator.getInstance().getValue<number>('MEMORY_CRITICAL_THRESHOLD')
    )
    this.setThresholdPercent(
      ResourceType.FILE_HANDLE,
      'warning',
      ConfigOrchestrator.getInstance().getValue<number>('FILE_HANDLE_WARNING_THRESHOLD')
    )
    this.setThresholdPercent(
      ResourceType.FILE_HANDLE,
      'critical',
      ConfigOrchestrator.getInstance().getValue<number>('FILE_HANDLE_CRITICAL_THRESHOLD')
    )
  }
  ...
  async cleanupResources(
    ...
    try {
        ...
        .filter(
          strategy =>
            ...
            targetResources.some(
              resource =>
                strategy.resourceTypes.includes(resource.type) && strategy.trigger(resource)
            )
        )
      ...
      const totalResourceReleased = results.reduce(
        (sum, result) => sum + (result.success ? result.resourceReleased : 0),
        0
      )
  ...
  getResourceStatistics(
    type?: ResourceType
  ): IResourceStatistics | Map<ResourceType, IResourceStatistics> {
    if (type) {
  ...
  private addToHistory(resource: IResourceInfo): void {
    ...
    const filteredHistory = history.filter(
      info => now - info.lastUpdate.getTime() < this.monitoringConfig.historyRetentionTime
    )
