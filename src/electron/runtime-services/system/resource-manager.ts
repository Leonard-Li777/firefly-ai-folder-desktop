...
import os from 'os';
import { connectionPool, concurrencyController } from '@yonuc/electron-llamaIndex-service';
...
export class ResourceManager extends EventEmitter {
  ...
  private applyConfigFromSettings(): void {
    ...
    const enableMonitoring = ConfigOrchestrator.getInstance().getValue<boolean>('ENABLE_HARDWARE_MONITORING');
  ...
  private updateThresholdsFromSettings(): void {
    this.setThresholdPercent(ResourceType.CPU, 'warning', ConfigOrchestrator.getInstance().getValue<number>('CPU_WARNING_THRESHOLD'));
    this.setThresholdPercent(ResourceType.CPU, 'critical', ConfigOrchestrator.getInstance().getValue<number>('CPU_CRITICAL_THRESHOLD'));
