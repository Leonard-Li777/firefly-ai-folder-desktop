...
export class FfmpegService extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    logger.info(LogCategory.SYSTEM, '[FfmpegService] FFmpeg 已在当前版本中禁用')
    return
    /*
    ...
    if (foundPath) {
      ...
      ;(globalThis as any)._firefly_ffmpeg_path = foundPath // 同步到全局
  ...
  async install(): Promise<boolean> {
    logger.info(LogCategory.SYSTEM, '[FfmpegService] FFmpeg 安装已禁用')
    return false
    ...
    try {
      ...
      const response = await nativeFetch(apiUrl, { timeout: 15000 })
      ...
      if (fs.existsSync(localPath)) {
        ...
        ;(globalThis as any)._firefly_ffmpeg_path = localPath
        if (process.platform !== 'win32') {
          try {
      ...
      if (fs.existsSync(zipPath)) {
        try {
          ...
          logger.warn(LogCategory.SYSTEM, `删除临时 ZIP 文件失败: ${zipPath}`, e)
        }
      }
  ...
  private async extractZip(zipPath: string, destFolder: string): Promise<void> {
    if (process.platform !== 'win32') {
      try {
        logger.info(LogCategory.SYSTEM, `[FfmpegService] 正在使用系统 unzip 命令解压: ${zipPath}`)
        execSync(`unzip -o -q "${zipPath}" -d "${destFolder}"`)
        logger.info(LogCategory.SYSTEM, `[FfmpegService] 系统 unzip 解压成功: ${zipPath}`)
        return
      } catch (err: unknown) {
        const message = err instanceof Error ? err.message : String(err)
        logger.warn(
          LogCategory.SYSTEM,
          `[FfmpegService] 系统 unzip 失败，将回退到 unzipper 库: ${message}`
        )
      }
    }
