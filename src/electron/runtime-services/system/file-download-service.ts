  ...
  async download(options: DownloadOptions): Promise<boolean> {
    ...
    if (fs.existsSync(destPath)) {
      ...
      if (remoteSize > 0 && localSize === remoteSize) {
        ...
        if (onProgress)
          onProgress({ percent: 100, receivedBytes: remoteSize, totalBytes: remoteSize, speed: 0 })
        return true
      }
    ...
    return new Promise(resolve => {
      ...
      fileStream.on('finish', () => {
        ...
        if (totalBytes > 0 && receivedBytes < totalBytes) {
          logger.error(
            LogCategory.SYSTEM,
            `[Download] 文件下载不完整: ${receivedBytes}/${totalBytes}`
          )
          if (fs.existsSync(destPath)) {
            try {
              fs.unlinkSync(destPath)
            } catch (e) {
              logger.warn(LogCategory.SYSTEM, `删除不完整的下载文件失败: ${destPath}`, e)
            }
          }
          resolve(false)
        } else {
