...
export class SystemHealthService extends EventEmitter {
  ...
  private initializeConfig(): void {
    // 从配置服务加载阈值
    const cpuWarning =
      ConfigOrchestrator.getInstance().getValue<number>('CPU_WARNING_THRESHOLD') || 70
    const cpuCritical =
      ConfigOrchestrator.getInstance().getValue<number>('CPU_CRITICAL_THRESHOLD') || 90
    const memWarning =
      ConfigOrchestrator.getInstance().getValue<number>('MEMORY_WARNING_THRESHOLD') || 75
    const memCritical =
      ConfigOrchestrator.getInstance().getValue<number>('MEMORY_CRITICAL_THRESHOLD') || 92
    const healthCheckInterval =
      ConfigOrchestrator.getInstance().getValue<number>('HEALTH_CHECK_INTERVAL') || 30000
    const maxRetries = ConfigOrchestrator.getInstance().getValue<number>('ERROR_MAX_RETRIES') || 3
    const retryDelay =
    ...
    this.config = {
      ...
      services: {
        ...
        timeout:
          ConfigOrchestrator.getInstance().getValue<number>('CONNECTION_IDLE_TIMEOUT') || 5000,
   ...
   * 更新监控间隔
   */
  public updateMonitoringInterval(newInterval: number): void {
      ...
      try {
        ...
        const status = await Promise.race([
          ...
          new Promise<ServiceHealthStatus>((_, reject) =>
            setTimeout(
              () => reject(new Error('Service check timeout')),
              this.config.services.timeout
            )
          )
        ])
  ...
  private async getResourceHealthStatus(): Promise<ResourceHealthStatus> {
    ...
    if (
      cpuUsage > this.config.monitoring.extremeThreshold.cpu ||
      ...
      cpuUsage > this.config.monitoring.warningThreshold.cpu ||
  ...
  private async attemptRecovery(healthStatus: SystemHealthStatus): Promise<void> {
    ...
    // 按优先级执行恢复策略
    const sortedStrategies = Array.from(this.recoveryStrategies.values()).sort(
   ...
   * 注册服务健康检查
   */
  public registerServiceHealthCheck(
    name: string,
  ...
  private rotateLogFile(): void {
    ...
    for (let i = this.config.logging.maxFiles - 1; i >= 1; i--) {
      const oldFile =
        i === 1 ? this.config.logging.filePath : path.join(logDir, `${logName}.${i - 1}.log`)
