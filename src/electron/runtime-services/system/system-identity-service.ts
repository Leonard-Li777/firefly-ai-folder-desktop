...
import type { IIdentityProvider } from '@firefly/types/identity-types'
    ...
    if (configId) {
      ...
      try {
        ...
        logger.error(
          ...
          'Failed to get system machine id, generating random hex id',
          { error: e }
        )
  ...
  getMachineId(): string {
    if (!this._machineId) {
      ...
      logger.warn(
        ...
        'SystemIdentityService not initialized, MACHINE_ID missing in config'
      )
