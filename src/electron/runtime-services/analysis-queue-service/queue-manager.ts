...
export class QueueManager {
    ...
    try {
      ...
      this.queue = rows
        ...
        .map(r => {
          ...
          let fileName = isDir ? r.dir_name : r.file_name;
          ...
          // 简化逻辑：
          // - itemType: 'file' 或 'directory'，用于区分项目类型
          // - type: 文件扩展名（如 '.jpg'），目录时为空字符串
  ...
  async addItems(inputs: EnqueueInput[], forceReanalyze = false): Promise<void> {
    ...
    const now = Date.now()
    // 使用标准路径作为 Map 的键，确保一致性
    const existingByPath = new Map(this.queue.filter(i => i.path).map(i => [path.resolve(i.path!), i as any]))
    let addedCount = 0
    let updatedCount = 0

    // 第一步：确保所有目录/文件记录存在于数据库中（异步）
    for (const file of inputs) {
      if (!file.path) continue
      const standardPath = path.resolve(file.path)
      const itemType = file.type === 'folder' ? 'directory' : 'file';

      if (itemType === 'directory') {
        // 确保目录记录存在
        const dir = databaseService.db?.prepare(`SELECT id FROM workspace_directories WHERE path = ?`).get(standardPath) as any;
        if (!dir) {
          // 查找所属工作区
          const ws = await databaseService.findRootWorkspaceDirectory(standardPath);
          if (ws && ws.id) {
            await databaseService.addDirectory(standardPath, ws.id)
          } else {
            logger.warn(LogCategory.ANALYSIS_QUEUE, `[分析队列] 目录 ${standardPath} 不属于任何工作区，跳过`)
          }
        }
      } else {
        // 确保文件记录存在
        const wf = databaseService.db?.prepare(`SELECT id FROM workspace_files WHERE path = ?`).get(standardPath) as any;
        if (!wf) {
          // 查找所属工作区
          const dirPath = path.dirname(standardPath)
          const ws = await databaseService.findRootWorkspaceDirectory(dirPath);
          if (ws && ws.id) {
            await databaseService.addFileFromPath(standardPath, '', ws.id)
          } else {
            logger.warn(LogCategory.ANALYSIS_QUEUE, `[分析队列] 文件 ${standardPath} 不属于任何工作区，跳过`)
          }
        }
      }
    }
    ...
    const transaction = databaseService.db?.transaction(() => {
      for (const file of inputs) {
        ...
        const standardPath = path.resolve(file.path)
        if (shouldIgnoreFile(standardPath, file.name, this.ignoreRules)) continue

        const exists = existingByPath.get(standardPath)
        if (exists) {
          // 关键逻辑：如果文件已在队列中且分析成功，则强制重新分析
          // 因为文件类型的文件不能从云端获取缓存，目录本身也无缓存
          ...
          if (shouldForceReanalyze) {
            ...
            logger.info(LogCategory.ANALYSIS_QUEUE, `[分析队列] 文件已在队列中 (原状态: ${oldStatus})，${reason}，重置为 pending: ${standardPath}`)
        ...
        if (itemType === 'directory') {
          const dir = databaseService.db?.prepare(`SELECT id, is_analyzed FROM workspace_directories WHERE path = ?`).get(standardPath) as any;
          itemId = dir?.id;
          isAlreadyAnalyzed = dir?.is_analyzed === 1;
        } else {
          const wf = databaseService.db?.prepare(`SELECT id, is_analyzed FROM workspace_files WHERE path = ?`).get(standardPath) as any;
          itemId = wf?.id;
        ...
        // 防御性检查：如果仍然找不到记录，跳过
        if (!itemId) {
          logger.error(LogCategory.ANALYSIS_QUEUE, `[分析队列] 无法找到路径 ${standardPath} 的数据库记录，跳过`)
          continue
        }

        // 关键逻辑：如果文件已被分析过，强制重新分析
        // 因为文件类型的文件不能从云端获取缓存，目录本身也无缓存
        const shouldForceReanalyze = forceReanalyze || file.forceReanalyze || isAlreadyAnalyzed
        
        if (isAlreadyAnalyzed && !forceReanalyze) {
          logger.info(LogCategory.ANALYSIS_QUEUE, `[分析队列] 文件已被分析过，自动设置强制重新分析: ${standardPath}`)
        }
        ...
        const fileExtension = itemType === 'file' 
          ? (file.type && file.type.startsWith('.') ? file.type : path.extname(standardPath).toLowerCase() || '')
          : ''
        ...
        // 入队时使用 item_id，获取数据库生成的自增 ID
        // 注意：使用同步方法，因为已经在 db.transaction() 内部
        ...
        // 更新为数据库返回的 ID（分析队列的自增 ID，可能与 itemId 不同）
        item.id = Number(dbId)
  ...
  deleteItemsByDirectory(directoryPath: string): void {
    if (!this.isInitialized) return
    const standardDirPath = path.resolve(directoryPath)
    const dirPathWithSep = standardDirPath.endsWith(path.sep) ? standardDirPath : `${standardDirPath}${path.sep}`

    const itemsToDelete = this.queue.filter(item => {
      if (!item.path) return false
      const itemPath = path.resolve(item.path)
      return itemPath === standardDirPath || itemPath.startsWith(dirPathWithSep)
    })
