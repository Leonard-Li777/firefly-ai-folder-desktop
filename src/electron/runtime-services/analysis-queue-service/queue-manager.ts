...
export class QueueManager {
  ...
  async loadFromDB(): Promise<void> {
    try {
        ...
        .map(r => {
          ...
          if (!filePath) {
            ...
            logger.warn(
              ...
              `[分析队列] 跳过路径为空的队列项: ${r.id} (类型: ${r.item_type})`
            )
            return null
          }
          ...
          if (r.analysis_stats) {
            try {
              const stats =
                typeof r.analysis_stats === 'string'
      ...
      if (skippedCount > 0) {
        logger.warn(
          LogCategory.ANALYSIS_QUEUE,
          `[分析队列] 共跳过 ${skippedCount} 个路径为空的队列项`
        )
        ...
        try {
          ...
          databaseService.db
            ?.prepare(
              `DELETE FROM analysis_queue WHERE item_type = 'file' AND item_id NOT IN (SELECT id FROM workspace_files)`
            )
            .run()
          // 删除目录类型但 item_id 不在 workspace_directories 中的记录
          databaseService.db
            ?.prepare(
              `DELETE FROM analysis_queue WHERE item_type = 'directory' AND item_id NOT IN (SELECT id FROM workspace_directories)`
            )
            .run()
        } catch (e) {
      ...
      if (analyzingItems.length > 0) {
        logger.info(
          LogCategory.ANALYSIS_QUEUE,
          '[分析队列] 重置',
          analyzingItems.length,
          '个 analyzing 状态的项目为 pending'
        )
  ...
  async addItems(inputs: EnqueueInput[], forceReanalyze = false): Promise<void> {
    ...
    const selectDirWorkspaceStmt = db?.prepare(
      `SELECT id, workspace_id FROM workspace_directories WHERE path = ?`
    )
    const selectFileWorkspaceStmt = db?.prepare(
      `SELECT id, workspace_id FROM workspace_files WHERE path = ?`
    )
    const selectDirAnalyzedStmt = db?.prepare(
      `SELECT id, is_analyzed FROM workspace_directories WHERE path = ? AND workspace_id = ?`
    )
    const selectFileAnalyzedStmt = db?.prepare(
      `SELECT id, is_analyzed FROM workspace_files WHERE path = ? AND workspace_id = ?`
    )
    ...
    for (const file of inputs) {
      ...
      if (itemType === 'directory') {
        // 尝试直接通过路径查找目录所属的工作区
        const dir = selectDirWorkspaceStmt?.get(nativePath) as any
        if (dir) {
        ...
        // 尝试直接通过路径查找文件所属的工作区
        const wf = selectFileWorkspaceStmt?.get(nativePath) as any
        if (wf) {
      ...
      if (!resolvedWorkspaceId) {
        logger.warn(
          LogCategory.ANALYSIS_QUEUE,
          `[分析队列] 项目 ${nativePath} 不属于任何已知工作区，跳过`
        )
        continue
      }
    ...
    const transaction = db?.transaction(() => {
      for (const file of resolvedInputs) {
        ...
        if (!itemId) {
          logger.error(
            LogCategory.ANALYSIS_QUEUE,
            `[分析队列] 无法在工作区 ${file.workspaceId} 中找到路径 ${file.nativePath} 的数据库记录，跳过`
          )
          continue
        }
        ...
        if (isAlreadyAnalyzed && !forceReanalyze) {
          logger.info(
            LogCategory.ANALYSIS_QUEUE,
            `[分析队列] 文件已被分析过，自动设置强制重新分析: ${file.nativePath}`
          )
        }
  ...
  clearPending(): void {
    ...
    logger.info(
      LogCategory.ANALYSIS_QUEUE,
      `[分析队列] 清空待处理项目，当前待处理数量: ${pendingCount}`
    )
    ...
    try {
      ...
      if (e.message?.includes('no such table')) {
        logger.warn(
          LogCategory.ANALYSIS_QUEUE,
          '[分析队列] analysis_queue 表不存在，跳过数据库清理'
        )
      } else {
  ...
  clearAll(): void {
    ...
    logger.info(
      LogCategory.ANALYSIS_QUEUE,
      `[分析队列] 清空所有队列项目，当前总数量: ${totalCount}`
    )
  ...
  markDirectoryCompleted(directoryPath: string): void {
    if (!this.isInitialized) return
    const dirItem = this.queue.find(
      item =>
    ...
    if (dirItem) {
      ...
      try {
        ...
        logger.info(
          LogCategory.ANALYSIS_QUEUE,
          `[分析队列] 同步将已完成上下文分析的目录队列项标记为 completed: ${directoryPath}`
        )
      } catch (e) {
  ...
  getSnapshot(currentItemId?: number, workspaceId?: number): AnalysisQueueSnapshot {
      ...
      for (const dbItem of dbItems) {
        if (!memoryIds.has(dbItem.id) && dbItem.status !== 'completed') {
          this.queue.push({
            ...
            isAnalyzed: (dbItem as any).is_analyzed === 1
          })
        }
      }
    } catch (e) {
