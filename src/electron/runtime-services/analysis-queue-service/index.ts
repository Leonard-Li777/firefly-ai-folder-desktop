...
export class AnalysisQueueService {
  ...
  async initialize(): Promise<void> {
    ...
    if (adapters) {
      this.qualityScoringService = new QualityScoringService(
        ...
        adapters.config,
        {
          getQualityScorePrompt: () => configService.getValue<string>('QUALITY_SCORE_PROMPT'),
          defaultScore: 3,
  ...
  private async expandDirectoryToQueue(directoryPath: string, forceReanalyze: boolean): Promise<void> {
    ...
    try {
      ...
      // 获取或分配 workspaceId
      let workspaceId = await databaseService.getWorkspaceIdByPath(path.dirname(filePath));
      if (!workspaceId) {
        logger.error(LogCategory.ANALYSIS_QUEUE, `[分析队列] 找不到工作空间归属: ${filePath}`)
        this.updateItemStatus(item.id, 'failed', 0, '工作空间不存在')
        return
      }
      ...
      if (needsNewHash) {
        ...
        fileFingerprint = await databaseService.calculateFileFingerprint(filePath)
      ...
      // --- 歌词处理增强逻辑 ---
      const isAudio = fileType === 'audio' || ['.mp3', '.wav', '.flac', '.m4a', '.aac', '.ogg'].some(ext => (filePath || '').toLowerCase().endsWith(ext));
      if (isAudio) {
   ...
   */
    ...
    try {
      ...
      const analysisStats = data.analysis_stats || data.analysisStats || null
  ...
  private async collectAnalysisStats(timer: PerformanceTimer): Promise<AnalysisStats> {
    try {
      ...
      const mode = configService.getValue<string>('AI_SERVICE_MODE')
      const modelId = configService.getValue<string>(mode === 'cloud' ? 'AI_CLOUD_SELECTED_MODEL_ID' : 'SELECTED_MODEL_ID')
