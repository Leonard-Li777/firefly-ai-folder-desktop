...
export class ErrorHandler {
  ...
  isCriticalError(errorType: AnalysisErrorType): boolean {
    ...
    return (
      criticalErrors.includes(errorType) && (this.errorRecoveryConfig.skipOnCriticalError ?? false)
    )
  }
  ...
  calculateRetryDelay(retryCount: number, errorType: AnalysisErrorType): number {
    ...
    const exponentialDelay =
      baseDelay * Math.pow(this.errorRecoveryConfig.backoffMultiplier ?? 2, retryCount)
   ...
   * 获取错误恢复建议（增强版）
   */
  private getEnhancedRecoveryAction(
  ...
  getErrorStatistics(): {
    ...
    const mostCommonError =
      (Object.entries(errorsByType).sort(([, a], [, b]) => b - a)[0]?.[0] as AnalysisErrorType) ||
      null
