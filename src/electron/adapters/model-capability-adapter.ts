...
import { LogCategory, logger, FileCategory, isCategory, getExtensionsByCategory, getFileCategory } from '@yonuc/shared'
...
import { modelCapabilityDetector, llamaModelManager } from '../runtime-services/llama/model-capability-detector'
import { CloudProviderService } from '@yonuc/electron-llamaIndex-service'
 ...
 * 模型能力适配器 (核心引擎专用)
 */
export class ModelCapabilityAdapter extends EventEmitter implements IModelCapabilityAdapter {
  ...
  async detectCapabilities(config: ExtendedAIServiceConfig): Promise<AICapabilities> {
    if (config.mode === 'cloud') {
      return CloudProviderService.getCapabilitiesForModel(config.cloud.provider, config.cloud.model || 'unknown');
    } else {
      return modelCapabilityDetector.detectLocalCapabilities(config);
    }
  }
  ...
  async getCapabilities(): Promise<AICapabilities> {
    ...
    if (aiServiceMode === 'cloud') {
      const provider = ConfigOrchestrator.getInstance().getValue<string>('AI_CLOUD_PROVIDER') as string;
      const model = ConfigOrchestrator.getInstance().getValue<string>('AI_CLOUD_SELECTED_MODEL_ID') as string;
      return CloudProviderService.getCapabilitiesForModel(provider, model || 'unknown');
    } else {
      const modelId = ConfigOrchestrator.getInstance().getValue<string>('SELECTED_MODEL_ID') as string;
      ...
      const serviceConfig: any = {
        ...
        local: {
          modelId: modelId,
          modelPath: '', // Model path usually resolved by LlamaServerService, not needed just for capability detection
          mmprojPath: mmprojFile ? mmprojFile.name : undefined, // simplified check
          contextSize: modelConfig?.contextLength || 4096,
          capabilities: modelConfig?.capabilities || []
        }
      };

      return modelCapabilityDetector.detectLocalCapabilities(serviceConfig);
    }
  }
  ...
  async checkFileTypeSupport(fileType: string, filePath?: string): Promise<boolean> {
    try {
      ...
      if (isCategory(filename, FileCategory.IMAGE)) return capabilities.supportsImage;
      if (isCategory(filename, FileCategory.AUDIO)) return capabilities.supportsAudio;
      if (isCategory(filename, FileCategory.VIDEO)) return capabilities.supportsVideo;
  ...
  isMultiModalModel(modelId?: string): boolean {
    ...
    if (aiServiceMode === 'cloud') {
      const provider = ConfigOrchestrator.getInstance().getValue<string>('AI_CLOUD_PROVIDER') as string;
      const model = modelId || ConfigOrchestrator.getInstance().getValue<string>('AI_CLOUD_SELECTED_MODEL_ID') as string;
      const caps = CloudProviderService.getCapabilitiesForModel(provider, model || 'unknown');
      return caps.supportsImage || caps.supportsAudio || caps.supportsVideo;
    } else {
      const currentModelId = modelId || ConfigOrchestrator.getInstance().getValue<string>('SELECTED_MODEL_ID') as string;
  ...
  async isMultimodalFileType(fileType: string): Promise<boolean> {
    try {
      const isMultimodal =
        isCategory(fileType, FileCategory.IMAGE) ||
  ...
  async getModelParameterSize(): Promise<number> {
    try {
      const aiServiceMode = ConfigOrchestrator.getInstance().getValue<string>('AI_SERVICE_MODE')
      if (aiServiceMode === 'cloud') {
        return 0
  ...
  async analyzeFileWithCapabilityAdaptation(
    ...
    try {
      logger.debug(LogCategory.MODEL_CAPABILITY_ADAPTER,
      ...
      logger.error(LogCategory.MODEL_CAPABILITY_ADAPTER,
  ...
  private async checkModelCapabilityMatch(
    ...
    // 检查能力匹配
    const checkMatch = (type: TModelCapabilityType, supported: boolean): IModelCapabilityMatch | null => {
      const typeCategoryMap: Record<TModelCapabilityType, FileCategory> = {
    ...
    ].filter(m => m !== null) as IModelCapabilityMatch[];
   ...
   * 查找部分匹配
   */
  private findPartialMatch(fileExtension: string, capabilities: AICapabilities): IModelCapabilityMatch | null {
    // 文件类型相似性映射
    ...
    const checkPartial = (type: TModelCapabilityType, supported: boolean): IModelCapabilityMatch | null => {
    ...
    ].filter(p => p !== null) as IModelCapabilityMatch[];
