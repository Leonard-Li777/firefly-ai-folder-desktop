...
export function recommendModelByHardware(memoryGB: number, hasGPU = false, vramGB?: number): string[] {
  ...
  if (qwen35Models.length === 0) {
    ...
    if (vramGB !== undefined) {
      // 允许显存稍微超出 (宽限系数 1.2)，因为 Ollama/llama.cpp 支持超过显存尺寸的模型加载
      const maxVramForModel = vramGB * 1.2;
      const eligibleModels = llamaConfigs.filter(model => {
        const modelVram = model.vramRequiredGB || model.hardwareRequirements?.minMemoryGB;
        return (modelVram || 0) <= maxVramForModel;
      });
...
export function getSupportedFileFormats(modelId: string): string[] {
  ...
  const modelConfigs = modelConfigService.loadModelConfig();
  const model = modelConfigs.find(m => m.id === modelId);
  if (!model) return [];

  return modelConfigService.getModelSupportedFormats(model);
}
...
export function isFileTypeSupported(modelId: string, fileExtension: string): boolean {
  ...
  return modelConfigService.getModelSupportedFormats(model).includes(fileExtension.toLowerCase().replace(/^\./, ''));
}
 ...
 */
function getLlamaModelsConfig(modelConfigs?: any[]): any[] {
 ...
 */
export function getLlamaModelConfig(modelId: string, modelConfigs?: any[]): any | null {
 ...
 */
export function getAllLlamaModelConfigs(modelConfigs?: any[]): any[] {
 ...
 */
export function getModelsByCapability(capabilityType: TModelCapabilityType, modelConfigs?: any[]): string[] {
  return getLlamaModelsConfig(modelConfigs)
    .filter(model => {
      const caps = model.capabilities || [];
      return caps.includes(capabilityType) || caps.some((c: any) => c === capabilityType || c.type === capabilityType);
    })
    .map(model => model.id);
}
...
export function recommendLlamaModelsByHardware(memoryGB: number, hasGPU = false, vramGB?: number): string[] {
  ...
  if (qwen35Models.length === 0) {
    ...
    if (vramGB !== undefined) {
      // 允许显存稍微超出 (宽限系数 1.2)，因为 Ollama/llama.cpp 支持超过显存尺寸的模型加载
      const maxVramForModel = vramGB * 1.2;
      const eligibleModels = llamaConfigs.filter(model => (model.vramRequiredGB || 0) <= maxVramForModel);
      if (eligibleModels.length > 0) {
        const sortedModels = [...eligibleModels].sort((a, b) => {
