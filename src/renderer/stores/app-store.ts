...
import { FileItem, DirectoryItem } from '@firefly/types/types'
...
export const useFileExplorerStore = create<FileExplorerState>((set, get) => ({
  ...
  toggleFileSelection: file =>
    set(state => {
      ...
      const isSelected = state.selectedFiles.some(
        f => f.path && file.path && isPathEqual(f.path, file.path)
      )
      return {
  ...
  removeFile: path =>
    set(state => {
      const isPathEqual =
        window.electronAPI?.utils?.isPathEqual || ((a: string, b: string) => a === b)
      return {
        files: state.files.filter(f => !f.path || !isPathEqual(f.path, path)),
        selectedFiles: state.selectedFiles.filter(f => !f.path || !isPathEqual(f.path, path))
      }
    }),
    ...
    set(state => {
      ...
      return {
        files: state.files.map(f =>
          f.path && isPathEqual(f.path, path) ? { ...f, ...updates } : f
        )
      }
    }),
  ...
  removeDirectory: path =>
    set(state => {
      const isPathEqual =
        window.electronAPI?.utils?.isPathEqual || ((a: string, b: string) => a === b)
      const newDirectories = state.directories.filter(d => !d.path || !isPathEqual(d.path, path))
      const newExpanded = new Set(state.expandedDirectories)
      newExpanded.delete(path)
