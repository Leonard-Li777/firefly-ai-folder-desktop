...
export interface ICloudModelConfigState {
  ...
  // 缓存的云端提供商预设配置（只需应用启动时加载一次）
  providersPresets: any[]
  // 当前加载 providersPresets 使用的语言（用于检测语言变化）
  providersPresetsLanguage: string | null
}
...
export interface ICloudModelConfigActions {
  ...
  // 重置状态
  reset: () => void
  // 设置 providersPresets 缓存
  setProvidersPresets: (presets: any[], language: string) => void
  // 获取 providersPresets 缓存（如果语言匹配）
  getProvidersPresetsIfMatch: (language: string) => any[] | null
}
...
export const useCloudModelConfigStore = create<TCloudModelConfigStore>()(
  subscribeWithSelector((set, get) => ({
    ...
    setProvidersPresets: (presets: any[], language: string) => {
      logger.debug(
        LogCategory.RENDERER,
        `缓存云端提供商预设: ${presets.length}个提供商, language=${language}`
      )
      set({ providersPresets: presets, providersPresetsLanguage: language })
    },

    getProvidersPresetsIfMatch: (language: string) => {
      const state = get()
      if (state.providersPresetsLanguage === language && state.providersPresets.length > 0) {
        return state.providersPresets
      }
      return null
    }
  }))
