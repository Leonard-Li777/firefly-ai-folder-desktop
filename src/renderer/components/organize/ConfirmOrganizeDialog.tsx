...
import { DirectoryNode, OrganizeType, FileInfoForAI } from '@firefly/types/organize-types'
...
interface ConfirmOrganizeDialogProps {
  ...
  onConfirmAnalyzedDirectory?: () => void
  isReadOnly?: boolean
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-card text-card-foreground rounded-lg shadow-xl max-w-2xl w-full p-6 max-h-[80vh] flex flex-col border border-border">
        ...
        {organizeType === 'quickOrganize' && !isReadOnly && (
                ...
                <div className="inline-block relative">
                  ...
                  {showVirtualDirTip && (
                    <div className="absolute bottom-full left-0 mb-2 p-3 bg-card text-card-foreground rounded shadow-lg border border-border z-50 w-64 text-xs">
                      {t(
                        '在虚拟目录页面左侧标签目录树中切换整理模式，勾选标签，点击导出虚拟目录按钮，定制化的生成你需要的目录结构，且不影响真实目录的文件结构'
                      )}
                    </div>
                  )}
                </div>
            ...
            {showInstructionInput && (
              <div className="mt-3">
                ...
                <div className="flex justify-end mt-3">
                  <Button size="sm" onClick={handleRegenerate} disabled={!instructionText.trim()}>
                    {t('重新整理')}
                  </Button>
                </div>
              </div>
        ...
        <h3 className="font-semibold mb-2">
          ...
          <span className="font-bold">
            {t('{fileCount} 个文件将被移动到新位置', { fileCount })}
          </span>
        </h3>
        ...
        <div className="flex justify-end gap-3 mt-4">
                ...
                if (window.electronAPI?.license) {
                  ...
                  if (statusResult.status !== 'AUTHORIZED') {
                    window.dispatchEvent(
                      new CustomEvent('app:unauthorized', { detail: statusResult })
                    )
                    return
                  }
                }
          ...
          {onConfirmAnalyzedDirectory && (
            <Button variant="default" onClick={onConfirmAnalyzedDirectory}>
              {t('存为已分析视角')}
            </Button>
          )}
