...
import { BatchProgress, FileInfoForAI } from '@firefly/types/organize-types'
...
export const AIOrganizeProgressDialog: React.FC<AIOrganizeProgressDialogProps> = ({
  ...
  const hasDirectories =
    currentResult && currentResult.directories && currentResult.directories.length > 0
      ...
      <div className="bg-card text-card-foreground rounded-lg shadow-xl max-w-4xl w-full p-6 max-h-[90vh] flex flex-col border border-border">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center">
            <MaterialIcon icon="auto_fix_high" className="text-blue-500 text-3xl mr-3" />
            <h2 className="text-xl font-bold">{t('AI正在分析整理方案...')}</h2>
          </div>
          <div className="flex items-center gap-2">
            {onPauseToggle && !isLastBatch && (
              <Button
                variant="outline"
                size="sm"
              ...
              >
                <MaterialIcon icon={isPaused ? 'play_arrow' : 'pause'} className="text-base" />
                {isPaused ? t('恢复') : t('暂停')}
              </Button>
            )}
            {onEnd && !isLastBatch && (
              <Button
                ...
                className="flex items-center gap-1 px-3 py-1.5 h-auto text-sm"
              >
                <MaterialIcon icon="stop" className="text-base" />
                {t('结束')}
              </Button>
            )}
            {onCancel && (
        ...
        <div className="mb-4">
          <div className="w-full bg-secondary rounded-full h-6 mb-2 overflow-hidden relative">
            <div
              className="bg-blue-500 h-6 w-full origin-left will-change-transform transition-transform duration-150 ease-out flex items-center justify-center text-white text-xs font-semibold"
              style={{ transform: `scaleX(${percentage / 100})` }}
            />
          </div>
          <p className="text-center text-sm font-semibold text-foreground">{percentage}%</p>
        </div>
        ...
        <div className="space-y-3 text-muted-foreground mb-6">
          ...
          <div className="flex items-center">
            ...
            <span>{t('{processedFiles}/{totalFiles} 个文件', { processedFiles, totalFiles })}</span>
          </div>
          ...
          <div className="mb-4 flex-1 overflow-y-auto">
            <div className="border-t pt-4">
              <div className="flex items-center mb-2">
                <MaterialIcon
                  icon="account_tree"
                  className="text-muted-foreground text-base mr-2"
                />
              ...
              <div className="border rounded p-3 bg-muted/30 dark:bg-muted/10 max-h-[300px] overflow-y-auto">
                <DirectoryTreePreview
                  ...
                  fileMap={fileMap}
                  isReadOnly={currentResult?.isReadOnly}
                />
              </div>
            </div>
          </div>
