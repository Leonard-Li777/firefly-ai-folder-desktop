...
export const UndoHistoryDialog: React.FC<UndoHistoryDialogProps> = ({
  ...
  const loadSessions = async () => {
    try {
      ...
      const loadedSessions =
        await window.electronAPI!.organizeRealDirectory.listSessions(workspaceDirectoryPath)
      setSessions(loadedSessions as any)
    } catch (error: any) {
      console.error('Failed to load sessions:', error)
      const message =
        error?.message?.replace(/^Error invoking remote method.*?: Error: /, '') || String(error)
      toast.error(t('加载历史记录失败: {error_message}', { error_message: message }))
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleUndo = async (sessionId: string) => {
    try {
      ...
      const message =
        error?.message?.replace(/^Error invoking remote method.*?: Error: /, '') || String(error)
      toast.error(t('撤销失败: {error_message}', { error_message: message }))
    } finally {
      setUndoing(null)
    }
  }
  ...
  const handleDelete = async (sessionId: string) => {
    try {
      ...
      const message =
        error?.message?.replace(/^Error invoking remote method.*?: Error: /, '') || String(error)
      toast.error(t('删除失败: {error_message}', { error_message: message }))
    }
  }
                    ...
                    <div className="flex-1">
                      ...
                      <div className="ml-7 space-y-1 text-sm text-muted-foreground">
                        <div className="flex items-center">
                          ...
                          <span>
                            {t('移动了 {count}', {
                              count: formatFileCount(session.fileMoves.length)
                            })}
                          </span>
                        </div>
                        {session.backupDir && (
                          <div className="flex items-center">
                            ...
                            <span className="truncate" title={session.backupDir}>
                              {t('备份位置: {backup_name}', {
                                backup_name: session.backupDir.split(/[/\\]/).pop()
                              })}
                            </span>
                          </div>
                        )}
