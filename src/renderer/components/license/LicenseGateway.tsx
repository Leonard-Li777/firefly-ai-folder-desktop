...
export const LicenseGateway: React.FC<LicenseGatewayProps> = ({ onActivated, status, error: initialError }) => {
                    ...
                    {!isOfflineMode ? (
                        <div className="space-y-6 py-4">
                            <div className="grid grid-cols-2 gap-4">
                                <Button size="lg" className="h-16 text-lg gap-2" onClick={handleRetryOnline} disabled={isActivating}>
                                    {isActivating ? <Loader2 className="w-5 h-5 animate-spin" /> : <Wifi className="h-5 w-5" />}
                                    {t('个人，我已打开网络')}
                                </Button>
                                <Button size="lg" variant="outline" className="h-16 text-lg gap-2" onClick={() => { setIsOfflineMode(true); setError(null); }} disabled={isActivating}>
                ...
                <CardFooter className="flex flex-col gap-4">
                    {isOfflineMode && (
                        <div className="flex w-full gap-4">
                            <Button variant="outline" className="w-full h-12 text-base" onClick={() => { setIsOfflineMode(false); setError(null); }}>
                                {t('返回')}
                            </Button>
                        </div>
                    )}
                    <p className="text-xs text-center text-muted-foreground px-8 italic">
                        {t('本软件受版权保护。合法授权后方可解锁全部智能文件整理功能。')}
                    </p>
                </CardFooter>
            </Card>
        </div>
