...
import { Copy, Mail, CheckCircle2, Loader2, ShieldAlert, Key, Sparkles, ExternalLink } from 'lucide-react';
...
export const EnterpriseLicenseForm: React.FC<EnterpriseLicenseFormProps> = ({ onActivated }) => {
    ...
    useEffect(() => {
        ...
        const updateTimer = () => {
            ...
            setTimeLeft(`${years}年-${months}月-${days}日 ${hours}:${minutes}:${seconds}`);
        };
    ...
    useEffect(() => {
        const init = async () => {
            if (window.electronAPI?.license) {
                // 1. 获取机器请求码
                const code = await window.electronAPI.license.getRequestCode();
                setRequestCode(code);

                // 2. 检查初始授权状态
                try {
                    const statusResult = await window.electronAPI.license.getStatus();
                    // 核心修复：只有非 ONLINE 类型的授权才被视为企业版成功
    ...
    const handleCopyRequestCode = () => {
        ...
        toast.success(t('请求码已复制到剪贴板'));
    };

    const handleOpenEnterprisePage = () => {
        openExternalLink('https://aifolder.iocn.cn/enterprise');
    };
        ...
        return (
            <div className="flex flex-col items-center justify-center py-12 space-y-8 animate-in zoom-in-95 duration-500">
                ...
                <div className="text-center space-y-3">
                    ...
                    <p className="text-muted-foreground font-bold text-lg">{t('授权验证成功，所有高级功能已解锁')}</p>
                </div>
                ...
                {expiryDate && (
                   <div className="mt-8 animate-in fade-in duration-1000 flex flex-col items-center space-y-4">
                       <Badge variant="outline" className="text-sm font-black py-1.5 px-6 rounded-2xl bg-primary/5 border-primary/20 text-primary">
                           {t('授权有效期至：{date}', { date: format(new Date(expiryDate), 'yyyy-MM-dd HH:mm:ss') })}
                       </Badge>
                       {timeLeft && (
        ...
        <div className="space-y-10 animate-in slide-in-from-right-4 duration-300 max-w-xl mx-auto py-6">
            <div className="space-y-5">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5 text-primary">
                        ...
                        <Label className="text-sm font-black tracking-widest uppercase opacity-80">{t('第一步：获取机器请求码')}</Label>
                    </div>
                    <Button 
                        variant="link" 
                        size="sm" 
                        className="text-[11px] font-bold h-auto p-0 flex items-center gap-1 opacity-70 hover:opacity-100"
                        onClick={handleOpenEnterprisePage}
                    >
                        {t('了解企业版专属功能')}
                ...
                <div className="flex items-start gap-2 p-3 bg-muted/30 rounded-xl border border-dashed border-border/80">
                    ...
                    <p className="text-[12px] text-muted-foreground font-bold leading-relaxed">
                        {t('请将此代码发送给管理员或开发者，以获取为您设备定制的企业版授权码。')}
                    </p>
                </div>
            </div>
            ...
            <div className="space-y-5">
                <div className="flex items-center gap-2.5 text-primary">
                    ...
                    <Label htmlFor="license" className="text-sm font-black tracking-widest uppercase opacity-80">{t('第二步：输入授权码')}</Label>
                </div>
            ...
            {error && (
                <Alert variant="destructive" className="border-2 shadow-lg bg-destructive/5 animate-in slide-in-from-top-2 rounded-2xl py-5">
                    <div className="flex items-start gap-4">
                        <ShieldAlert className="h-6 w-6 shrink-0 mt-0.5" />
                        <div className="space-y-1">
                            <AlertTitle className="font-black text-base leading-none">{t('授权验证失败')}</AlertTitle>
                                <AlertDescription className="font-bold opacity-90 text-sm">{error}</AlertDescription>
                            </div>
                        </div>
                    </Alert>
                )}
            <div className="pt-4 border-dashed border-border/40 space-y-8">
                <div className="flex items-center gap-5 bg-gradient-to-br from-muted/50 to-muted/20 p-5 rounded-3xl border border-border/30">
                    <div className="p-4 bg-background shadow-md rounded-2xl shrink-0 ring-1 ring-border/5">
                        <Mail className="w-7 h-7 text-primary" />
                    </div>
                    <div className="min-w-0 flex-1 space-y-2">
                        <p className="font-black text-sm tracking-tight text-primary/80 uppercase">{t('获取授权 / 咨询')}</p>
                        <div className="flex flex-wrap items-center gap-x-12 gap-y-1 text-base font-mono lowercase tracking-tighter">
                           ...
                           <div className="flex items-center gap-2">
                               ...
                               <span className="text-foreground font-black">seaeye777@qq.com</span>
                           </div>
                        </div>
                    </div>
                </div>
...
const Badge: React.FC<{ children: React.ReactNode, variant?: string, className?: string }> = ({ children, className }) => (
    <div className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 border border-transparent ${className}`}>
        {children}
    </div>
);
