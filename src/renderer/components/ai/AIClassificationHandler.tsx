  ...
  useEffect(() => {
    ...
    const handleClassificationRequest = async (
      ...
      try {
        ...
        if ((status as unknown as string) !== 'loaded' && (status as unknown as string) !== 'initialized') {
          logger.info(
            LogCategory.AI_SERVICE,
    ...
    const performLocalAIClassification = async (
      ...
      try {
        ...
        if (audioExtensions.includes(ext)) {
          logger.info(LogCategory.AI_SERVICE, '[AI Classification Handler] 检测到音频文件，准备转换 base64:', filename)
          try {
            ...
            logger.error(LogCategory.AI_SERVICE, '[AI Classification Handler] 音频转换 base64 失败:', err)
          }
        } else if (imageExtensions.includes(ext)) {
          logger.info(LogCategory.AI_SERVICE, '[AI Classification Handler] 检测到图像文件，准备转换 base64:', filename)
          try {
            const base64 = await (window.electronAPI!.utils as any).readFileBase64(filename)
            images.push(base64.replace(/^data:image\/[a-z]+;base64,/, ''))
          } catch (err) {
            logger.error(LogCategory.AI_SERVICE, '[AI Classification Handler] 图像转换 base64 失败:', err)
          }
        }
    ...
    // 注册监听器
    const unsubscribe = window.electronAPI!.onAIClassificationRequest(handleClassificationRequest as any)
