        ...
        <div className="space-y-5">
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-1.5">
              ...
              <HelpTooltip
                content={t(
                  '根据需求选择不同模式，AI分析耗时最长但精度最高；简单分类最快但不支持AI生成描述。'
                )}
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div
                ...
                className={`relative overflow-hidden flex flex-col p-3.5 rounded-lg border-2 cursor-pointer transition-all ${
                  (getConfigValue<string>('ANALYSIS_MODE') ?? 'full') === 'simple'
              ...
              >
                ...
                <p className="text-xs text-muted-foreground mt-2 leading-relaxed">
                  {t(
                    '【简单分类】 + 全面AI分析，包含智能命名、标签提取、简短描述、质量评分与多模态内容描述、lrc（歌词）等。'
                  )}
                </p>
              </div>
            </div>
              ...
              <div className="flex items-center justify-between p-3 rounded-lg border bg-card">
                <div className="space-y-1">
                  <div className="flex items-center gap-1">
                    ...
                    <HelpTooltip
                      content={t(
                        '开启后会对PDF、Office文档中的所有页图片进行OCR识别，Office缩略图提取也依赖OCR开启。注：仅支持小于10MB的文件，大于10MB将自动跳过OCR，且开启会让分析变慢或超时'
                      )}
                    />
                  </div>
                  <p className="text-xs text-muted-foreground">
                    {localEnableDocumentOcr
                      ? t('开启对于office分析变慢，pdf很快')
                      : t('关闭，则仅提取已有文本层')}
                  </p>
                </div>
                <Switch
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1 pr-4">
                <div className="flex items-center gap-1">
                  ...
                  <HelpTooltip
                    content={t(
                      '安装LibreOffice，有助于提高分页提取文档内容的成功率，以及对老旧格式的支持'
                    )}
                  />
                </div>
                <p className="text-sm text-muted-foreground mt-1">
                  {t('系统转换工具，提升 Office 及 PDF 等办公文档的内容提取与分页处理性能。')}
                </p>
                      ...
                      >
                        <div className="p-2.5 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded text-xs text-amber-700 dark:text-amber-300 space-y-1">
                          ...
                          <ul className="ml-4 list-disc space-y-0.5">
                            <li>{t('请先将 LibreOffice 的安装路径添加进系统的 PATH 环境变量')}</li>
                ...
                {!checkingLibreOffice && libreOfficeInstalled === false && (
                  <Button size="sm" className="mt-3.5" onClick={handleOpenLibreOfficeDownload}>
