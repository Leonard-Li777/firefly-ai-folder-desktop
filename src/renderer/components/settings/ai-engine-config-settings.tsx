          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-border bg-muted/30">
                <th className="p-4 text-xs font-black text-muted-foreground uppercase">
                  {t('AI 引擎')}
                </th>
                <th className="p-4 text-xs font-black text-muted-foreground uppercase">
                  {t('适配类型')}
                ...
                <th className="p-4 text-xs font-black text-muted-foreground uppercase text-right">
                  {t('当前引擎')}
                </th>
              </tr>
            </thead>
                ...
                tableRows.map((row, index) => (
                  ...
                  >
                    <td className="p-4 text-sm flex items-center gap-2">
                      <Cpu
                        className={`h-4 w-4 ${row.isCurrent ? 'text-primary' : 'text-muted-foreground/60'}`}
                      />
                      <span>{row.name}</span>
                    </td>
                    ...
                    <td className="p-4 text-sm text-right">
                      {row.isCurrent ? (
                        <Badge className="bg-primary text-primary-foreground font-black px-2.5 py-0.5 rounded-full shadow-sm animate-pulse">
                          {t('当前引擎')}
                        </Badge>
                      ) : (
                        <Button
            ...
            <div className="flex items-start gap-3 flex-1 min-w-0">
              ...
              <div className="flex-1 min-w-0">
                <Label className="text-sm font-black flex items-center gap-2 flex-wrap">
                  <span>{t('模型思考模式')}</span>
                  <span className="text-[10px] font-bold text-purple-600 dark:text-purple-500 bg-purple-500/10 px-1.5 py-0.5 rounded-md border border-purple-500/20">
                    {t('可选优化')}
                  </span>
                </Label>
