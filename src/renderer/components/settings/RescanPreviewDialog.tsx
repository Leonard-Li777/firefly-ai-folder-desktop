...
import type { IIgnoreRule } from '@firefly/types/settings-types'
import { shouldIgnoreFileByRules } from '@firefly/shared'
...
export const RescanPreviewDialog: React.FC<RescanPreviewDialogProps> = ({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const rules = await (window.electronAPI as any).getAnalysisIgnoreRules()
        if (!canceled) {
          // 类型断言：将后端返回的类型转换为 IIgnoreRule[]
          setIgnoreRules(Array.isArray(rules) ? (rules as unknown as IIgnoreRule[]) : [])
        }
      } catch {
        if (!canceled) {
          setIgnoreRules([])
  ...
  const isDirectoryPartiallySelected = useCallback(
    (node: TreeNode): boolean => {
      if (node.type === 'file') return false
      const allFiles = getAllFilesInDirectory(node)
  ...
  const handleAddToQueue = useCallback(async () => {
    ...
    try {
      ...
      const filesForQueue = filesToAdd.map(change => ({
        path: change.path,
        name: change.name,
        size: change.size,
        type: change.type
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {fileChanges.length === 0 ? (
            <div className="text-center py-8">
              <MaterialIcon
                icon="check_circle"
                className="text-6xl text-green-600 dark:text-green-400 mb-3"
              />
            ...
            <>
              <div className="mb-4 p-4 bg-muted/50 dark:bg-muted/50 rounded-lg">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="text-muted-foreground dark:text-muted-foreground">
                    {t('已选中')} <span className="font-semibold">{selectedPaths.size}</span> /{' '}
                    {fileChanges.length}
                  </span>
                </div>
              </div>
              <div className="mb-4 p-3 bg-muted/30 dark:bg-muted/30 rounded text-xs flex items-center gap-4">
                ...
                <span className="flex items-center gap-1">
                  <span className="w-3 h-3 rounded bg-yellow-600 dark:bg-yellow-400"></span>
                  <span className="text-muted-foreground dark:text-muted-foreground">
                    {t('修改文件')}
                  </span>
                </span>
          ...
          {fileChanges.length > 0 && (
            <>
              ...
              <button
                ...
                className="px-4 py-2 bg-primary text-primary-foreground rounded hover:bg-primary/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
              >
                {isAdding ? (
                  <>
                  ...
                  <>
                    <MaterialIcon icon="playlist_add" className="text-base mr-1" />
                    {t('加入队列 ({count})', { count: selectedPaths.size })}
                  </>
                )}
              </button>
            </>
