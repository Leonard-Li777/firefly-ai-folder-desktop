...
export const AIModelSettings: React.FC = () => {
  ...
  const activeModelKey = useMemo(
    () => resolveActiveModelKey(models as any, config?.selectedModelId),
                  ...
                  {!groupedModels[s] || groupedModels[s].length === 0 ? (
                    ...
                    groupedModels[s].map(model => (
                      <ModelCardItem
                        ...
                        model={model}
                        isDownloaded={modelDownloadStatus[`${model.id}@${model.source}`]}
                        isActive={activeModelKey === `${model.id}@${model.source || 'default'}`}
