...
import { formatFileSize } from '@yonuc/shared'
...
export const MonitoringSettings: React.FC = () => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      // 如果删除的是当前工作目录，清空当前选择
      if (isDeletingCurrentDir && typeof window.electronAPI?.setCurrentWorkspaceDirectory === 'function') {
  ...
  const handleRescanDirectory = async (directory: WorkspaceDirectory) => {
    ...
    try {
      ...
      if (typeof window.electronAPI?.rescanWorkspaceDirectory === 'function') {
        ...
        // 检查是否有变更
        const hasChanges = ((result as any).stats?.newFiles?.length || 0) > 0 || ((result as any).stats?.modifiedFiles?.length || 0) > 0
                      ...
                      <div className="flex items-center gap-2 ml-4">
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            try {
                              // 主动触发：先扫描空文件夹
                              const scanResult = await window.electronAPI!.emptyFolder.scan(directory.path)
