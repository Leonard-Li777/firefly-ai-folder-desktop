    ...
    <Card className={`bg-card text-card-foreground shadow-sm  p-2 ${className}`}>
      <div className="flex items-start ring-input justify-between gap-2">
        <div className="flex-1">
          ...
          {/* 进度条：使用 GPU 硬件加速的 transform: scaleX，避免 width 变动引发高频 CPU 重排 */}
          <div
            className="h-2 w-full rounded bg-muted mb-3 overflow-hidden relative"
            role="progressbar"
            aria-valuemin={0}
            aria-valuemax={100}
          ...
          >
            <div
              className={`h-full w-full rounded origin-left will-change-transform transition-transform duration-150 ease-out ${
                error ? 'bg-destructive' : status === 'retrying' ? 'bg-orange-500' : 'bg-primary'
              }`}
              style={{
                transform: `scaleX(${Math.min(1, Math.max(0, currentPercent / 100))})`
              }}
            />
          </div>
