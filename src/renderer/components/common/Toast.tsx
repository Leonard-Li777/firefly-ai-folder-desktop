  ...
  return (
    ...
    >
      <MaterialIcon
        ...
        className={`text-xl ${iconColors[toast.type]} ${toast.type === 'loading' ? 'animate-spin' : ''}`}
      />
      <div className="flex flex-col gap-1">
        ...
        <div className="flex gap-2">
          ...
          {toast.action?.secondaryLabel && (
            <button
              onClick={e => {
                e.stopPropagation()
                toast.action?.onSecondaryClick?.()
                onClose()
              }}
              className="text-xs font-medium opacity-70 hover:underline text-left"
            >
              {toast.action.secondaryLabel}
            </button>
          )}
