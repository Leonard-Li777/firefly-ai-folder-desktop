  ...
  return (
    <div className="flex flex-col items-center justify-center h-full w-full bg-background p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl w-full">
        ...
        <Card
          className={cn(
            'transition-colors border-4 border-border/50 group',
            isEnterprise
              ? 'opacity-30 cursor-not-allowed grayscale'
              : 'hover:border-primary/50 cursor-pointer'
          )}
          onClick={() => !isEnterprise && onAddWorkspaceDirectory('SPEEDY')}
          title={isEnterprise ? t('企业版暂不支持极速目录') : undefined}
        >
          <CardHeader>
                ...
                <div className="absolute left-full top-0 ml-2 hidden group-hover/tooltip:block z-50 w-80 max-w-[80vw] p-4 bg-popover dark:bg-popover text-popover-foreground dark:text-popover-foreground text-xs rounded-md shadow-lg border border-border dark:border-border animate-in fade-in-0 zoom-in-95">
                  <div className="space-y-3">
                    ...
                    <p className="leading-relaxed">
                      <strong className="text-foreground">{t('萤核智能文件夹')}</strong>
                      {t('始终将隐私保护作为核心设计原则，坚持')}
                      <strong className="text-foreground">{t('数据本地优先')}</strong>
                      {t(
                        '，切实保障用户隐私安全，其核心技术原理与行业常见的哈希（Hash）机制逻辑一致。'
                      )}
                    </p>
                    <div className="space-y-2">
                      <p className="leading-relaxed">
                        <strong className="text-foreground">{t('简单来说')}</strong>
                        {t(
                          '，系统会通过加密算法为每个文件生成唯一的特征标识（长度 16-128 字符），相当于文件的专属 ID。'
                        )}
                      </p>
                      <ul className="list-disc list-inside space-y-1.5 ml-1">
                        <li>
                          {t('生成这一 ID 的')}
                          <strong className="text-foreground">{t('唯一前提')}</strong>
                          {t(
                            '，是用户本地持有该文件并完成算法计算，应用不会向任何第三方泄露该文件 ID。仅凭借此 ID，才可向服务器请求对应 ID 的分析数据，无其他获取途径。'
                          )}
                        </li>
                        <li>
                          {t('即便在')}
                          <strong className="text-foreground">{t('极端假设下')}</strong>
                          {t(
                            '（服务器被突破），第三方仅能获取文件分析数据与对应 ID，且不同平台的 ID 算法完全独立，与百度网盘等平台的文件 ID 无任何关联，无法跨平台匹配原文件。'
                          )}
                        </li>
                        <li>
                          {t('同时，')}
                          <strong className="text-foreground">{t('服务器仅存储分析类数据')}</strong>
                          {t(
                            '：包括智能文件名、标签、内容摘要、评分及理由、元数据等，也就是大家在右侧属性面板可见的内容，绝不会上传、存储文件本体。'
                          )}
                        </li>
                      </ul>
                    </div>
