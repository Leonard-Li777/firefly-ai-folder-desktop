...
export function getStageLabel(status: AnalysisQueueItem['status'], stage?: number): string {
  if (status === 'analyzing') {
    ...
    switch (s) {
      case 1:
        return t('Stage 1 特征识别中')
      case 2:
        return t('Stage 2 内容提取中')
      case 3:
        return t('Stage 3 质量评分中')
      case 4:
        return t('Stage 4 维度分析中')
      default:
        return t('Stage 1 特征识别中')
    }
  }
  ...
  switch (stage) {
    case 1:
      return t('Stage 1 特征识别')
    case 2:
      return t('Stage 2 内容提取')
    case 3:
      return t('Stage 3 质量评分')
    case 4:
      return t('Stage 4 维度分析')
    default:
      return t('未分析')
  }
}
...
export function AnalysisQueueContent({
  ...
  // 响应式读取分析模式：用于按分析模式判断文件完成 stage（Sample->1, Document->2, Full->4）
  const analysisMode = useSettingsStore(
    s => (s.getConfigValue<string>('ANALYSIS_MODE') as string) ?? 'full'
  )
    ...
    <div className="flex flex-col h-full w-full bg-background text-foreground overflow-hidden">
      ...
      >
        ...
        >
          ...
          {(() => {
            const currentMode =
              useSettingsStore.getState().getConfigValue<string>('ANALYSIS_MODE') ?? 'full'
            return (
              <span
