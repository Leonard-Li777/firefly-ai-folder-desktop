...
function Row({ item }: { item: AnalysisQueueItem }) {
  ...
  return (
    <div className="grid grid-cols-12 items-center px-3 border-b border-border/50 last:border-0 hover:bg-accent/50 transition-colors" style={{ height: ROW_HEIGHT }}>
      <div className="col-span-2 truncate text-foreground font-medium" title={item.name}>{item.name}</div>
      <div className="col-span-2 truncate text-muted-foreground text-xs" title={item.path}>{item.path}</div>
      <div className="col-span-1 text-sm"><StatusBadge status={item.status} /></div>
      <div className="col-span-3 truncate text-xs text-destructive/80 font-medium" title={failedReason || ''}>
      ...
      <div className="col-span-1 flex justify-center text-xs">
        ...
        ) : item.status === 'completed' && item.analysisStats?.durationMs ? (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-sm animate-fade-in text-primary">
      <div className="bg-popover w-[900px] max-w-[95vw] rounded-xl shadow-2xl p-6 flex flex-col animate-scale-in">
        ...
        <div className="grid grid-cols-12 text-xs font-medium text-muted-foreground px-3 py-2 border-b border-border/50">
          <div className="col-span-2">{t('文件名')}</div>
          <div className="col-span-2">{t('路径')}</div>
          <div className="col-span-1">{t('状态')}</div>
          <div className="col-span-3">{t('失败原因')}</div>
          <div className="col-span-1 text-center">{t('缓存/耗时')}</div>
