...
import { FileItem as FileType, DirectoryItem } from '@firefly/types'
...
const TreeNode: React.FC<TreeNodeProps> = ({
  ...
  const swapFileNameDisplay = config.swapFileNameDisplay ?? false
  const isSelected =
    !isDirectory && selectedFiles.some(f => f.path && item.path && isPathEqual(f.path, item.path))
  ...
  const handleSelect = useCallback(
    (pathOrEvent?: string | React.MouseEvent, checked?: boolean) => {
      // 如果是事件对象，忽略参数
      ...
      if (!isDirectory) {
        ...
        const newSelected = shouldSelect
          ...
          : selectedFiles.filter(
              f => !f.path || !fileItem.path || !isPathEqual(f.path, fileItem.path)
            )
  ...
  if (isDirectory) {
    ...
    return (
      <div className="tree-node">
        ...
        {isExpanded && <div className="tree-node-children">{/* 子节点将由父组件递归渲染 */}</div>}
      </div>
    )
  }

  const fileItem = item as FileType
  const primaryName = swapFileNameDisplay ? fileItem.name : fileItem.smartName || fileItem.name
  return (
    <div className="tree-node">
      <div
        className={cn(
