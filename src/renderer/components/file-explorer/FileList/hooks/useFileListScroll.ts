...
export const useFileListScroll = ({
  ...
  const handleScroll = useCallback(
    (params: any) => {
      ...
      if (params) {
        ...
        else if (params.currentTarget && params.currentTarget.scrollTop !== undefined) {
          offset = Number(params.currentTarget.scrollTop)
          targetEl = params.currentTarget
        } else if (params.target && params.target.scrollTop !== undefined) {
          offset = Number(params.target.scrollTop)
      ...
      // 如果有真实 DOM 元素，基于真实可滚动高度精准检测触底
      if (targetEl && targetEl.scrollHeight > 0) {
        const remaining = targetEl.scrollHeight - targetEl.scrollTop - targetEl.clientHeight
        if (remaining < 350) {
          onLoadMore()
          return
        }
      ...
      if (offset + containerSize.height >= totalHeight - 300) {
        onLoadMore()
      }
    },
  ...
  const handleGridScroll = useCallback(
    (params: any) => {
      ...
      if (offset + containerSize.height >= totalHeight - 400) {
        onLoadMore()
      }
    },
