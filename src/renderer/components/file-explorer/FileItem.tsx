...
import { formatFileSize as sharedFormatFileSize, FileCategory, isCategory } from '@yonuc/shared';
...
export const FileItem: React.FC<ExtendedFileItemProps> = memo(({
  ...
  const displayUrl = React.useMemo(() => {
    ...
    if (thumbnailPath && workspaceDirectoryPath) {
      baseUrl = `file://${normalizeForCache(workspaceDirectoryPath)}/${normalizeForCache(thumbnailPath)}`;
    }
    // 如果没有缩略图，但本身是图片，则显示原图
    else if (isImageFile(extension)) {
      baseUrl = `file://${normalizeForCache(path)}`;
    } else {
      return null;
    }
    ...
    return (
      <div
        className={cn(
          ...
          viewMode === 'list' ? 'flex items-center px-3 py-2 border-b border-border/50' : 'flex flex-col items-center p-4 rounded-xl border border-border/50',
      ...
      >
        <div className={cn(
          "absolute z-10",
          viewMode === 'list' ? "left-2" : "top-3 left-3 opacity-0 group-hover:opacity-100 transition-opacity",
        ...
        <div className={cn(
          ...
          viewMode === 'list' ? "w-10 h-10 ml-8 mr-3" : "w-24 h-24 mb-3 transition-transform group-hover:scale-105"
        )}>
          {displayUrl ? (
            <img
              src={displayUrl}
            ...
            <div className={viewMode === 'list' ? "scale-50" : "scale-100"}>
              {icon}
            </div>
          )}
        </div>
