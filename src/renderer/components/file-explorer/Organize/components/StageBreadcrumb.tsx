        ...
        return (
          <React.Fragment key={step.key}>
            <span
              title={step.label}
              className={cn(
                'flex items-center gap-1 px-2.5 py-1 rounded-full font-semibold transition-all duration-300 min-w-0 shrink overflow-hidden',
                isActive
            ...
            >
              <MaterialIcon
                icon={idx < currentIdx ? 'check' : step.icon}
            ...
            {idx < steps.length - 1 && (
              <MaterialIcon
                icon="chevron_right"
                className="text-[13px] text-muted-foreground/30 shrink-0"
              />
            )}
          </React.Fragment>
        )
