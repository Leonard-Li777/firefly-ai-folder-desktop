...
export const FileDetailsPanel: React.FC<any> = ({
  ...
  const getThumbnailUrl = (): string | null => {
    ...
    if (!finalWorkspaceDirectoryPath) return null
  ...
  const availableTabs = useMemo(() => {
    ...
    const tabs: any[] = []
    if (analysisResult.qualityScore != null || analysisResult.qualityReasoning)
      tabs.push({ id: 'quality', label: t('质量评分'), icon: 'star_rate' })
    if (analysisResult.multimodalContent?.trim())
      tabs.push({ id: 'multimodal', label: t('多模态描述'), icon: 'visibility' })
          ...
          {analysisResult &&
            ...
            (analysisResult.smartName ||
              ...
              <>
                ...
                {analysisResult.groupingReason && (
                  <p
                    className={cn(
                      'text-xs text-muted-foreground mt-1.5 leading-relaxed text-left',
                      maskClass
                    )}
                  >
                    {analysisResult.groupingReason}
