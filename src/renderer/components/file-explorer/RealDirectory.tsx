...
export const RealDirectory: React.FC<RealDirectoryProps> = ({
  ...
  const handleFileSelect = (
    ...
    const { isPathEqual, normalizeForCache } = window.electronAPI.utils

    const normalizePath = (p?: string) => (p ? normalizeForCache(p) : '')
    const resolveByPath = (path: string) => {
      const { directories, files } = useFileExplorerStore.getState()
      const all = [...directories, ...files]
      return all.find(it => isPathEqual((it as any).path, path)) || null
    }
    ...
    if (isFromCheckbox) {
      ...
      if (filesToRemove.length > 0) {
        ...
        updatedSelection = updatedSelection.filter(
          f => !pathsToRemove.has(normalizePath(f.path))
        )
      }
  ...
  useEffect(() => {
    const loadWorkspaceDirectories = async (isInitial = false) => {
      try {
        ...
        const foundInList = directories.find(d => d.path === currentInStore?.path)
  ...
  useEffect(() => {
    if (currentWorkspaceDirectory) {
      ...
      // 允许路径是工作区路径的子目录
      const normalize = (p: string) => window.electronAPI?.utils?.getPlatform?.() === 'win32' ? p.toLowerCase() : p
      const nPath = normalize(currentPath)
      const nWorkspacePath = normalize(currentWorkspaceDirectory.path)
      const isWin = window.electronAPI?.utils?.getPlatform?.() === 'win32'
      const nWorkspacePrefix = nWorkspacePath.endsWith(isWin ? '\\' : '/') ? nWorkspacePath : nWorkspacePath + (isWin ? '\\' : '/')
      
      if (!currentPath || (nPath !== nWorkspacePath && !nPath.startsWith(nWorkspacePrefix))) {
        logger.info(LogCategory.RENDERER, '检测到工作区变更或非法路径，重置当前路径:', currentWorkspaceDirectory.path)
  ...
  const handleAddWorkspaceDirectory = async (type: 'SPEEDY' | 'PRIVATE' = 'SPEEDY') => {
    ...
    try {
      ...
      if (!result.canceled && result.filePaths.length > 0) {
        ...
        // 5. 自动选中新创建的目录
        // 注意：无需手动重新加载 getAllWorkspaceDirectories，
        // 因为 onWorkspaceDirectoriesUpdated 监听器会自动触发 loadWorkspaceDirectories
        logger.info(LogCategory.RENDERER, '自动选中新创建的工作目录:', newDirectory.name)
        
        // 这里的 handleSelectWorkspaceDirectory 会同时更新后端状态和前端 Store
        // 并且 loadWorkspaceDirectories 里的逻辑会保证不覆盖这次更新
  ...
  const isUpButtonDisabled = () => {
    ...
    const normalizedWorkspacePath = currentWorkspaceDirectory.path
      ? normalize(currentWorkspaceDirectory.path)
      : ''
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 flex overflow-hidden">
            ...
            <div className="flex-1 min-w-0 flex flex-col overflow-hidden">
              ...
              <div className="flex-shrink-0 border-b border-border px-3 py-2 flex items-center justify-between bg-card">
                <div className="flex items-center space-x-2 flex-1 min-w-0">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (currentPath) {
                        window.electronAPI!.utils.openPathInExplorer(currentPath).catch((error: any) => {
                          logger.error(LogCategory.RENDERER, '无法在资源管理器中打开目录:', error)
                ...
                <div className="flex items-center space-x-2 text-foreground dark:text-foreground">
                  ...
                  >
                    <Button
                      variant={snapshot.running ? "default" : "outline"}
                      size="sm"
                      className={`gap-1.5 transition-all duration-700 relative
                        ${
                          snapshot.running
                            ? 'animate-pulse shadow-[0_0_15px_-5px_rgba(168,85,247,0.5)]'
                            : ''
                        }`}
                      onClick={openModal}
                    >
                      ...
                      <span className="font-bold">
                        {t('分析队列 ({length}/{snapshotLength})', {
                          length: snapshot.items.filter(i => i.status !== 'completed').length,
              ...
              <div className="flex-1 overflow-hidden bg-muted dark:bg-muted">
                  <FileList
