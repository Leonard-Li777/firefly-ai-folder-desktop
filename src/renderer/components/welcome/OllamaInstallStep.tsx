...
import { LogCategory, logger, formatFileSize } from '@firefly/shared'
...
export function OllamaInstallStep({ onComplete, onBack, onSwitchToCloud }: OllamaInstallStepProps) {
  ...
  const refreshMirrorState = useCallback(async () => {
    try {
      ...
      logger.debug(
        ...
        `[MirrorDebug] 尝试获取镜像源状态... getMirror接口是否存在: ${!!getMirror}`
      )
      if (getMirror) {
      ...
      logger.error(
        LogCategory.RENDERER,
        '[MirrorDebug] Failed to sync mirror state via dedicated API:',
        e
      )
    }
    return 'cn'
  ...
  const handleInstall = useCallback(async () => {
    ...
    logger.info(
      LogCategory.RENDERER,
      `[MirrorDebug] handleInstall 启动，当前使用的镜像源: ${currentMirror}`
    )
  ...
  useEffect(() => {
    ...
    const unsubscribeGeneral = window.electronAPI?.onConfigChange?.(config => {
      logger.debug(
        LogCategory.RENDERER,
        `[MirrorDebug] 监听到通用配置变更，DOWNLOAD_MIRROR: ${config.DOWNLOAD_MIRROR}`
      )
      refreshMirrorState()
    })
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        if (currentMirror === 'global') {
          ...
          for (let i = 0; i < 10; i++) {
            ...
            if (currentMirror === 'cn') {
              logger.info(
                LogCategory.RENDERER,
                '[MirrorDebug] 初始化：成功检测到镜像源自动切换为 CN'
              )
              break
            }
          }
        ...
        logger.info(
          LogCategory.RENDERER,
          `[MirrorDebug] 探测周期结束，最终使用镜像: ${currentMirror}`
        )
                    ...
                    {platform === 'linux' ? (
                      <div className="h-64 overflow-y-auto rounded-2xl bg-slate-900 p-6 text-[13px] font-mono text-slate-50 shadow-2xl ring-1 ring-white/10">
                        ...
                        {installLogs.map((log, index) => (
                          <div
                            key={index}
                            className="break-words whitespace-pre-wrap py-1 border-b border-slate-800/30 last:border-0 opacity-90"
                          >
                          ...
                          <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
                            ...
                            <div className="grid grid-cols-3 gap-3 mt-6">
                              ...
                              <div className="bg-slate-50/80 p-4 rounded-2xl border border-slate-100/80 flex flex-col gap-1">
                                ...
                                <span className="font-black text-slate-700 text-sm tabular-nums tracking-tight">
                                  {downloadInfo.receivedMB} / {downloadInfo.totalMB} MB
                                </span>
                              </div>
                              <div className="bg-slate-50/80 p-4 rounded-2xl border border-slate-100/80 flex flex-col gap-1">
                                <span className="text-[10px] text-slate-400 font-black uppercase tracking-wider">
                                ...
                                <span className="font-black text-sky-600 text-sm tabular-nums tracking-tight">
                                  {downloadInfo.eta}
                                </span>
                              </div>
                            </div>
              ...
              <CardContent className="p-0">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <Badge
                      variant="secondary"
                      className="bg-sky-100 text-sky-700 text-[9px] font-black uppercase tracking-widest px-2 py-0.5 border-none"
                    >
                      {t('备选方案')}
                    </Badge>
                  </div>
                  <div className="flex items-center gap-1.5 opacity-60">
                    <div
                      className={`w-2 h-2 rounded-full ${mirror === 'cn' ? 'bg-emerald-500' : 'bg-slate-400'}`}
                    />
                      ...
                      <div className="p-5 bg-amber-50/50 rounded-2xl border-2 border-amber-100/50 flex gap-4 items-start shadow-inner">
                        ...
                        <div>
                          ...
                          <p className="text-[13px] text-amber-800/80 leading-snug font-medium">
                            {t('复制链接并使用')}{' '}
                            <strong className="text-amber-950 underline decoration-amber-500/40 font-black">
                              {t('迅雷、IDM 或 Motrix')}
                            </strong>{' '}
                            {t('等专业工具，下载速度可提升至 10 倍以上！')}
                          </p>
                        </div>
                      </div>
                    </div>
              ...
              <Card className="rounded-3xl bg-rose-50 shadow-sm border-rose-100 p-8 text-slate-900 animate-in shake-1 duration-500">
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-rose-600 font-bold mt-1 opacity-80">
                      {errorMessage}
                    </p>
                  </div>
                </div>
          ...
          <div className="p-10">
            <AlertDialogHeader className="flex flex-col items-center text-center">
              ...
              <AlertDialogDescription className="text-[16px] text-slate-500 leading-relaxed font-medium px-4">
                {t('下载已顺利完成！请点击刚刚弹出的Ollama官方安装程序窗口完成安装。')}
              </AlertDialogDescription>
            </AlertDialogHeader>
                ...
                <div className="flex items-start gap-5">
                  ...
                  <div>
                    ...
                    <p className="text-sky-700 font-bold text-sm leading-relaxed">
                      {t('安装完成后，点击下方按钮关闭应用。请手动重新启动应用以进行下一步。')}
                    </p>
                  </div>
                </div>
              </div>
