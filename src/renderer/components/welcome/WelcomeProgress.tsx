...
const STEPS = {
  ...
  MODE: { id: 'mode', name: t_static('模式选择') },
  ...
  DOWNLOAD: { id: 'download', name: t_static('资源下载') },
  ...
  OLLAMA_DOWNLOAD: { id: 'ollama_download', name: t_static('模型下载') },
...
export function WelcomeProgress({ currentStep }: WelcomeProgressProps) {
  ...
  if (isLlamaAny && modelMode === 'local') {
    steps = [STEPS.LANGUAGE, STEPS.MODE, STEPS.STORAGE, STEPS.DOWNLOAD, STEPS.COMPLETE]
  }
  ...
  if (isOllama && modelMode === 'local') {
    ...
    // Llama Any (cpp/file) 模式映射: 1->0, 2->1, 4->2, 5->3, 6->4 (跳过 3)
    if (currentStep === 1) activeIndex = 0
    else if (currentStep === 2) activeIndex = 1
    else if (currentStep === 4) activeIndex = 2
    else if (currentStep === 5) activeIndex = 3
    else if (currentStep === 6) activeIndex = 4
