...
export function OllamaModelDownloadStep({ onNext, onBack }: OllamaModelDownloadStepProps) {
  ...
  const startDownload = async () => {
    ...
    try {
      ...
      if (typeof window.electronAPI?.onOllamaModelProgress === "function") {
          ...
          <Card className="p-8">
            {status === 'downloading' && (
              <div className="text-center">
                ...
                <p className="text-sm text-slate-500 mb-2">{t('如果下载慢，开启VPN或游戏加速器可以显著改善下载速度')}</p>
            ...
            {status === 'error' && (
              <div className="text-center">
                ...
                <p className="text-sm text-slate-500 mb-6">{t('{errorMessage}，请稍后再试或选择其它模型下载', { errorMessage })}</p>
