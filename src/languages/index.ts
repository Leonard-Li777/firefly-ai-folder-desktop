...
// 导出基本的翻译函数
// 特殊修改：使用函数包装导出，避免在模块加载阶段提前触发 appScope 访问导致主进程崩溃
export const t = (...args: any[]) => (i18nScope.t as any)(...args)
// 特殊修改：使用函数包装导出，避免在模块加载阶段提前触发 appScope 访问导致主进程崩溃 (TypeError: Cannot read properties of undefined (reading 'scope'))
export const $t = (...args: any[]) => (i18nScope.$t as any)(...args)
// 特殊修改：使用函数包装导出，确保 Translate 组件在 manager 准备就绪后再初始化
export const Translate = (props: any) => (i18nScope.Translate as any)(props)

// 导出语言管理函数
// 注意：VoerkaI18nScope 实例上没有 getLanguage()/changeLanguage() 方法，
// 当前语言应读取 activeLanguage getter，切换语言应调用 change()（useVoerkaI18n hook 内部即 change.bind(scope)）
export const getLanguage = () => (i18nScope as any).activeLanguage
export const changeLanguage = (lang: string) => (i18nScope as any).change(lang)
